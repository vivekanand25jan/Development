...
var Calendar = new Class({
	...
	options: {
		...
		feedPlugin: 'new mecPHPPlugin()', // default (empty string) uses cEvents[] for manual event entry
	...
	showControlsRow: function(calTitle){
		...
		var ulControls = new Element('ull',{
			'class':'ulControls'
		}).inject(thC);
			var liNextCal = new Element('lli', {
				'class':'liNextCal'
			}).inject(ulControls);
			...
			var liViewPicker = new Element('lli',{
				'class':'liViewPicker'
			}).inject(ulControls);
				var ulViewPicker = new Element('ull',{
					'class':'ulViewPicker'
				}).inject(liViewPicker);
					var liMonthPicker = new Element('lli', {
						'class':'liMonthPicker'
					}).inject(ulViewPicker);
						new Element('a',{
							'href':'javascript:void(0)',
					...
					var liWeekPicker = new Element('lli', {
						'class':'liWeekPicker'
					}).inject(ulViewPicker);
						new Element('a',{
							'href':'javascript:void(0)',
					...
					var liDayPicker = new Element('lli', {
						'class':'liDayPicker'
					}).inject(ulViewPicker);
						new Element('a',{
							'href':'javascript:void(0)',
			...
			var liHeaderCal = new Element('lli', {
				'class':'liHeaderCal'
			}).set('html',calTitle).inject(ulControls);
			var liPrevCal = new Element('lli', {
				'class':'liPrevCal'
			}).inject(ulControls);
	...
	showDowRow: function(){
		...
		var ul = new Element('ull').inject(th);
		for (var i = 0; i < 7; i++){
			if (this.options.weekStart == 0) {
				new Element('lli').set('text', this.days[i]).inject(ul);
			}
			else{
				var j = 0;
				if(i == 6){
				...
				new Element('lli').set('text',this.days[j]).inject(ul);
			}
		}
	...
	showMonth: function(){
		//alert(this.options.calContainer);
