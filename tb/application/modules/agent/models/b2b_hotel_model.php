...
function get_currecy_detail($val)
{
		...
		function inser_customer_book_hotelpro_trans_hotel($trans_id,$ConfirmationNumbervalue,$userid,$val_last)
	{
	 	...
	 	$this->db->query("UPDATE transaction_details SET prn_no='$ConfirmationNumbervalue', 	booking_number='$ConfirmationNumbervalue',  hotel_booking_id='$val_last'  WHERE customer_contact_details_id ='$trans_id'");
...
function gta_bed_fac1()
	...
	function dummy_insert_permenant_result_pro($hotel_code,$hotel_name,$city,$star,$description,$location,$latitude,$longitude,$api,$address,$tel_val,$fax_val,$Chainval)
	{
			...
			$this->db->insert('api_hotel_detail_permanent',$data);
		   return $this->db->insert_id();
...
SELECT MIN(t.total_cost) as low_cost FROM api_hotel_detail_t t, api_permanent p WHERE t.hotel_code = p.hotel_code AND session_id = '$ses_id' $where group by  t.hotel_code ) as tab";
	...
	{
		//chitta
		//$ses_id = 't95tf94606an4onbi3qsh9jem3';
		
		$agent_markup = $this->session->userdata('markup');
		$agent_id = $this->session->userdata('agent_id');
		
		//echo "xx:".$agent_markup;
		$display_perpage = 10;
		$start_pos = $display_perpage * ($page-1);
		$where = '';
		...
		$agent_markup_str = '';
		if (!empty($agent_markup)) {
			$agent_markup_str = "($agent_markup/100) * ((MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * MIN(t.total_cost))) +" . 
					"((SELECT (m.commission/100) AS commission FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"(MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"MIN(t.total_cost))))) + ";
		}
		//echo "mm:".$agent_markup_str;
		
		if (empty($fac)) {
			$select = "SELECT SQL_CALC_FOUND_ROWS *, MIN(t.total_cost) AS low_cost_price, ". 
					"ROUND(".$agent_markup_str."(MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * MIN(t.total_cost))) +" . 
					"((SELECT (m.commission/100) AS commission FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"(MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"MIN(t.total_cost)))),2) AS low_cost " . 
					"FROM api_hotel_detail_t t, api_permanent p WHERE t.hotel_code = p.hotel_code AND session_id = '$ses_id' $where GROUP BY t.hotel_code $order";
		} else {
			$where.= " and MATCH(f.fac) AGAINST ('$fac' IN BOOLEAN MODE)";
			$select = "SELECT SQL_CALC_FOUND_ROWS *, MIN(t.total_cost) as low_cost_price, ".
			"ROUND(".$agent_markup_str."(MIN(t.total_cost) + " . 
			...
			"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
			"MIN(t.total_cost)))),2) AS low_cost " . 
			"FROM api_hotel_detail_t t, api_permanent p, facilities f WHERE t.hotel_code = p.hotel_code and t.hotel_code = f.hotel_code AND t.session_id = '$ses_id' $where GROUP BY t.hotel_code $order";
		...
		if (empty($fac)) {
			$select2 = "select MIN(low_cost) as minVal, MAX(low_cost) as maxVal from ( " . 
				"SELECT ROUND(".$agent_markup_str."(MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * MIN(t.total_cost))) +" . 
					"((SELECT (m.commission/100) AS commission FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"(MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"MIN(t.total_cost)))),2) AS low_cost FROM api_hotel_detail_t t, api_permanent p WHERE t.hotel_code = p.hotel_code AND t.session_id = '$ses_id' $where group by  t.hotel_code ) as tab";
			...
			$select2 = "select MIN(low_cost) as minVal, MAX(low_cost) as maxVal from ( " . 
				"SELECT ROUND(".$agent_markup_str."(MIN(t.total_cost) + " . 
					...
					"(MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = $agent_id AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"MIN(t.total_cost)))),2) AS low_cost FROM api_hotel_detail_t t, api_permanent p, facilities f WHERE t.hotel_code = p.hotel_code and t.hotel_code = f.hotel_code AND t.session_id = '$ses_id' $where group by  t.hotel_code ) as tab";
		}
...
SELECT MIN(t.total_cost) as low_cost FROM api_hotel_detail_t t, api_permanent p WHERE t.hotel_code = p.hotel_code AND session_id = '$ses_id' $where group by  t.hotel_code ) as tab";
	...
	function fetch_search_result_for_page($ses_id, $page=1, $minVal=0, $maxVal=0, $minStar=0, $maxStar=0, $finval=0)
	{
		$display_perpage = 10;
		$start_pos = $display_perpage * ($page-1);
		$where = '';
...
//			
	...
	function fetch_search_result_for_page2($ses_id, $page, $minVal=0, $maxVal=0, $minStar=0, $maxStar=5, $fac='', $sorting='')
	{
		...
		if (empty($fac)) {
			$select = "SELECT SQL_CALC_FOUND_ROWS *, MIN(t.total_cost) AS low_cost_price, ". 
					"ROUND((MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = 1 AND m.api_id = a.api_id AND a.api_name = t.api) * MIN(t.total_cost))) +" . 
					"((SELECT (m.commission/100) AS commission FROM agent_markup m, api a WHERE m.agent_id = 1 AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"(MIN(t.total_cost) + " . 
					"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = 1 AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
					"MIN(t.total_cost)))),2) AS low_cost " . 
					"FROM api_hotel_detail_t t, api_permanent p WHERE t.hotel_code = p.hotel_code AND session_id = '$ses_id' $where GROUP BY t.hotel_code $order LIMIT $start_pos, $display_perpage";
		} else {
			$where.= " and MATCH(f.fac) AGAINST ('$fac' IN BOOLEAN MODE)";
			$select = "SELECT SQL_CALC_FOUND_ROWS *, MIN(t.total_cost) as low_cost_price, ".
			"ROUND((MIN(t.total_cost) + " . 
			...
			"((SELECT (m.markup/100) AS markup FROM agent_markup m, api a WHERE m.agent_id = 1 AND m.api_id = a.api_id AND a.api_name = t.api) * " . 
			"MIN(t.total_cost)))),2) AS low_cost " . 
			"FROM api_hotel_detail_t t, api_permanent p, facilities f WHERE t.hotel_code = p.hotel_code and t.hotel_code = f.hotel_code AND t.session_id = '$ses_id' $where GROUP BY t.hotel_code $order LIMIT $start_pos, $display_perpage";
...
GROUP BY `api_permanent_facility`.`fac`";
	...
	{
		// ccccc
	/*$que="SELECT `api_permanent_facility`.`fac`,`api_permanent_facility`.`hotel_code`
FROM (
`api_hotel_detail_t`
)
JOIN `api_permanent_facility` ON `api_hotel_detail_t`.`hotel_code` = `api_permanent_facility`.`hotel_code`
WHERE `session_id` = '".$ses_id."'
AND `fac_type` = 'hotel' AND `fac` IN ('".$val."') group by `api_permanent_facility`.`hotel_code`
";*/
		//$que="SELECT f.fac, f.hotel_code FROM api_permanent_facility f, api_hotel_detail_t t where f.fac_type = 'hotel' AND  MATCH(f.fac) AGAINST ('bar,Gym') and f.hotel_code = t.hotel_code and t.session_id='".$ses_id."' group by f.hotel_code";
		
		//$que="SELECT f.fac, f.hotel_code FROM api_permanent_facility f, api_hotel_detail_t t where f.fac_type = 'hotel' AND  MATCH(f.fac) AGAINST ('bar,Gym') and f.hotel_code = t.hotel_code group by f.hotel_code";

if (!empty($val)) {
$que = "SELECT t.*,p.*, f.*, f.fac, f.hotel_code, MIN(t.total_cost) as low_cost FROM api_permanent_facility f, api_hotel_detail_t t, api_permanent p WHERE p.hotel_code = t.hotel_code and f.fac != '' and f.fac_type = 'hotel' and t.hotel_code = f.hotel_code and t.session_id = '$ses_id' and MATCH(f.fac) AGAINST ('$val') GROUP BY t.hotel_code ORDER BY low_cost ASC";
}
else {
