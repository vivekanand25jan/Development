...
function display_child_count2(value)
{
	...
	if($_SESSION['fav_hotel_detail'][$i] != 'images')
	{
        ...
        <div class="hotel_left_panel_cont_1_in">
          ...
          <div class="hotel_view_right_cont"><a class="hotel_link_new" href="<?php echo WEB_URL.'hotel/hotel_detail/'.$_SESSION['fav_hotel_detail'][$i]; ?>"><?php echo $detailss->hotel_name; ?></a>
            <p> <?php
				$star = $detailss->star;
				if($star==1)
											   {
     ...
     <div>
                ...
                <div class="wi550_R_400"><?php
				 ...
				 $total_price = 0;
				  ...
				  for($k=0;$k<count($a);$k++)
				  {
					...
					/*if ($this->session->userdata($b->api)) {
					$cost = $cost + $b->total_cost + (($this->session->userdata($b->api)/100)*$b->total_cost);
					} else {
						$cost = $cost + $b->total_cost;
					}
					...
					if ($this->session->userdata('agent_logged_in')) {
						if ($this->session->userdata($b->api)) {
							$cost = $cost + (($this->session->userdata($b->api)/100)*$cost);
							//echo "agent:".$total_price;
						}
					} else {
						if ($this->session->userdata('staff_logged_in')) {
						$cost = $cost + (($this->session->userdata('staff_commission')/100)*$cost);
				  ...
				  $total_price = number_format($this->session->userdata('currency_value') * $cost, 2, '.', ',');
        ...
        <div class="wi550_c">
                ...
                <div class="wi550_R_400"><?php
				  ...
				  ?>  <?php echo $total_price . ' ' . $this->session->userdata('currency'); ?></div>
        </div>
        <div class="wi550_c">
       ...
       <div class="forth_text">
        ...
        <br />
       <form name="book" action="<?php echo WEB_URL ?>b2b_hotel/pre_booking/<?php echo $result_id; ?>" method="post">
		  ...
		  for($i=0;$i< count($room_info); $i++)
		  {
            ...
            <div style="float:left; margin-top:10px;">
              <div style="float:left">
                <div> <strong>Full Guest Name *</strong> </div>
                <div>
                  <input type="text" name="fname[]" class="wi382_C_text_fie"  />
                </div>
                <div class="clr"></div>
              </div>
...
<textarea name="req" cols="" rows=""  class="wi382_C_text_fie_area"></textarea>
		...
		if($api != 'travco')
		{
			//echo "cc:".$cost . ",". $balance_credit;
			if ($balance_credit < $cost) { ?>
			<b>Current balance is less than booking amount in your account</b> <br />
			<input type="image" disabled="disabled" src="<?php echo WEB_DIR ?>images/contact_btn.jpg" width="106" height="26" />
			<?php } else { ?>
