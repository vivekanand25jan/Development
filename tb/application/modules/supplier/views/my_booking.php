...
function cancel_booking(booking_number,id,api,pnr)
{
	if(confirm("Are you sure, you want to Cancel the booking?")) {
		...
		$.ajax({
		  url:'<?php echo WEB_URL; ?>index/cancel_booking/', 
		  data: "booking_number="+booking_number+"&id="+id+"&api="+api+"&pnr="+pnr,
		  dataType: "json",
		  type: 'POST',
		  success: function(result){
			if (result == 'success') {
				window.location = "<?php echo WEB_URL; ?>index/my_booking/";
			}
...
<style>
 ...
 <script type="text/javascript" src="<?php echo WEB_DIR ?>gui/ajax/ajaxsbmt.js"></script>
  ...
  $("#result").load("<?php print WEB_URL ;?>index/my_booking_search_news/"+fdate+"/"+tdate+"/"+booking_status+"/"+passenger_name+"/"+booking_number);
  ...
  $("#result1").load("<?php print WEB_URL ;?>index/my_booking_search_news_b/"+fdate+"/"+tdate+"/"+booking_status+"/"+passenger_name+"/"+booking_number);
...
<table width="920" border="0" align="left" cellpadding="0" cellspacing="0" style="margin:15px 0 0 15px; border:1px solid #ccc;">
  ...
  <tr>
    ...
    <td align="left" valign="top" class="my_profile_name_ex_tab_whit"><?php echo $result[$i]->amount; ?> </td>
    ...
    <!--<td align="left" valign="top" class="my_profile_name_ex_tab_whit_ex"><?php
							...
							echo ' <a href="'.WEB_URL.'index/cancel_booking/'.$result[$i]->transaction_details_id.'">Cancel</a>'; 
							}
...
<table width="920" border="0" align="left" cellpadding="0" cellspacing="0" style="margin:15px 0 0 15px; border:1px solid #ccc;">
  ...
  <tr>
    ...
    <td align="left" valign="top" class="my_profile_name_ex_tab_whit_ex"><?php
							...
							echo ' <a href="'.WEB_URL.'index/cancel_booking/'.$result_p[$i]->transaction_details_id.'">Cancel</a>'; 
							}
							 if($result[$i]->agent_mode!='cash')
							 {
							 ?>
					 |	<a  href="javascript:void(0);" onclick="JavaScript:newPopup('<?php echo WEB_URL . 'index/voucher_print/' . $result_p[$i]->hotel_booking_id; ?>'); " >Voucher</a> | 
                            <a  href="javascript:void(0);" onclick="JavaScript:newPopup1('<?php echo WEB_URL . 'index/invoice_print/' . $result_p[$i]->hotel_booking_id; ?>');" >Invoice</a>
							
							| 
                             
                            <a  href="javascript:void(0);" onclick="JavaScript:newPopup2('<?php echo WEB_URL  . 'index/send_voucher_email/' . $result_p[$i]->hotel_booking_id; ?>');" >Mail</a>
