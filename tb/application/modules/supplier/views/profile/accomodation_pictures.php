...
jQuery(function() {
	$('#uploadFile').uploadify(	{
		...
		'buttonImage' : '<?php echo WEB_DIR ?>images/upload_bt.png',
	...
	function(data){  
		...
		for(var i=0; i<data.acc_images.length; i++)
		{
			if(data.acc_images[i].status =="1"){
				$("#getPhotos").prepend('<div class="image-box" id="pic1"><div><img src="<?php echo WEB_DIR ?>supplier_hotels_images/'+data.acc_images[i].image_name+'" width="165" height="120" border="0" style="margin:4px;" /><div class="checkbox-bg"><span style="float:left;"><img src="<?php echo WEB_DIR ?>images/active.png" width="14" height="14" border="0" style="vertical-align:top;cursor:pointer;" onclick="return status_pic('+data.acc_images[i].sup_accomodation_pictures_id+',0);"></span><span class="img-fix"> <img src="<?php echo WEB_DIR ?>images/b2b-booking-icon.png" width="14" height="14" border="0" style="margin-left:5px;margin-top:5px;cursor:pointer;" onclick="return delete_pic('+data.acc_images[i].sup_accomodation_pictures_id+');"/></span> </div></div></div>');
			}
			else {
				$("#getPhotos").prepend('<div class="image-box" id="pic1"><div><img src="<?php echo WEB_DIR ?>supplier_hotels_images/'+data.acc_images[i].image_name+'" width="165" height="120" border="0" style="margin:4px;" /><div class="checkbox-bg"><span style="float:left;"><img src="<?php echo WEB_DIR ?>images/inactive.png" width="14" height="14" border="0" style="vertical-align:top;cursor:pointer;" onclick="return status_pic('+data.acc_images[i].sup_accomodation_pictures_id+',1);"></span><span class="img-fix"> <img src="<?php echo WEB_DIR ?>images/b2b-booking-icon.png" width="14" height="14" border="0" style="margin-left:5px;margin-top:5px;cursor:pointer;" onclick="return delete_pic('+data.acc_images[i].sup_accomodation_pictures_id+');"/></span> </div></div></div>');
			}
		}
		}
		else
...
<li><a href="<?php echo WEB_URL;?>index/accomodation_pictures/<?php echo $this->uri->segment(3)?>" id="tabs_active"> Photo Gallery </a></li>
  ...
  <div id="containerdount"  class="tab1" style="padding-top:30px; margin-top:20px;">
    ...
    <div class="mediaUploadBar" align="right">
     Upload a valid file with extension JPG, JPEG, PNG, GIF.&nbsp;
   	 <div class="uploadData" style="float:right; margin-top:-4px;"><input type="file" name="uploadFile" id="uploadFile" /> </div>
 	</div>
    </div>
