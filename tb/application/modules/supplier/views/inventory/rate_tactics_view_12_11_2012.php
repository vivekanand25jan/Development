...
<table width="942" border="0" align="left" cellpadding="0" cellspacing="0" style="margin:15px 0 0 17px; ">
  <tr>
    <td align="right" valign="top" class="" colspan="9" height="30"><a href="<?php echo site_url('index/add_rate_plan_view/')?>/<?php echo $this->uri->segment(3); ?>" style="color:#099; font-weight:bold;">Add room rate</a></td>
  </tr>
   <tr>
    ...
    <td align="left" valign="top" class="my_profile_name_ex_tab" colspan="2">Period</td>
    ...
    <?php
	  ...
	  for($j=0;$j<count($cate_types);$j++)
	  {	
	    $cate_type = $cate_types[$j]->category_type;
	    $result = $this->Supplier_Model->rate_tactics_list_by_category($supplier_id,$prop_id,$cate_type);
  ...
  <tr>
    ...
    <td align="left" valign="top" class="my_profile_name_ex_tab_whit"><?php if($i==0) echo $j+1; ?></td>
   <?php if($i==0) { ?>
    <td align="left" valign="center" class="my_profile_name_ex_tab_whit" rowspan="<?php echo count($result);?>">
    
    
	<?php  $category_name = $this->Supplier_Model->get_category_name($result[0]->category_type); 
  ...
  <?php } ?>
    ...
    <td align="left" valign="top" class="my_profile_name_ex_tab_whit"><?php $fd = $result[$i]->room_avail_date_from; $fds = explode("-",$fd); $from_date = $fds[2].'-'.$fds[1].'-'.$fds[0]; $td = $result[$i]->room_avail_date_to; $tds = explode("-",$td); $to_date = $tds[2].'-'.$tds[1].'-'.$tds[0]; echo $from_date.' To '.$to_date;	
	 ?>
     <td align="left" valign="top" class="my_profile_name_ex_tab_whit">
     <a href="<?php echo site_url('index/add_rate_plan_view/')?>/<?php echo $this->uri->segment(3); ?>/<?php echo $result[$i]->category_type; ?>" title="Add Season Period">
    ...
    <td align="left" valign="top" class="my_profile_name_ex_tab_whit"><a href='<?php echo WEB_URL; ?>index/promotion_details/<?php echo $this->uri->segment(3); ?>/<?php echo $result[$i]->sup_rate_tactics_id; ?>' style="text-decoration:none;color:#099; text-transform:capitalize; float:left;" >Promotion</a></td>
