...
if($api_no!=''){
		...
		if( $c_sd!= '' &&  $c_ed!= ''){
		 ...
		 $select = "SELECT h.*, t.*, c.first_name FROM hotel_booking_info h left join transaction_details t on h.customer_contact_details_id = t.customer_contact_details_id left join customer_contact_details c on c.customer_contact_details_id = h.customer_contact_details_id where t.product_id=2  $where";
	...
	function get_currecy_detail()
{
	...
	{		
		...
		echo	$select = "SELECT * FROM agent where agent_id = '$id'";
...
function generateValidationCode()
{
	...
	function agent_info($agent_id)
	{
		...
		$select = "SELECT a.name, i.balance_credit FROM agent a left join agent_acc_info i on i.agent_id = a.agent_id WHERE a.agent_id = $agent_id  limit 1";
		$query=$this->db->query($select);
		if ($query->num_rows() > 0)
		{
			return $query->row();
...
function cancel_booking($booking_number)
	...
	//----- Create New Supplier -----/
	function add_supplier($name, $company_name, $address, $country, $city, $postal_code, $office_phone, $mobile_no,$currency, $email_id, $password, $hotel_logo)
	{
		$data = array(
			'email_id' => $email_id,
			'password' => $password,
