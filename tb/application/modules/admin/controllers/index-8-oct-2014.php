	...
	function registration_page()
	{
		...
		else
		{
		   ...
		   $agent_logo  = $_FILES['agent_logo_admin']['name'];
                   $markets = implode(',',$this->input->post('market_id'));
		   ...
		   if ($this->admin_model->add_agent($name, $company_name, $address, $country, $city, $postal_code, $office_phone, $mobile_no,$currency, $email, $password,$a_type,$cc_id, $agent_logo,$markets)) {
...
mysql_select_db("travel_bay",$conn);
	...
	function view_book_detail($id)
	{
		...
		$con_id = $data['result_view'][0]->customer_contact_details_id;
	...
	function invoice_print($book_id)
	{
		...
		$data['result_view']=$this->admin_model->view_trans_detail_id_new($book_id);
		...
		$data['pass_info']=$this->admin_model->pass_info_detail($con_id_pass);
		 ...
		 $hotel_id = $data['result_view'][0]->hotel_code;
...
'.ucfirst($city).', '.ucfirst($country).'</td>
  ...
  <tr>
	...
	function my_account_update_agent($id)
	{
		...
		$a_type= $_POST['a_type'];
		$cc_id= $_POST['cc_id'];
		$market= implode(',',$_POST['market_id']);
	//	print_r( $_FILES);die();
		if(isset($_FILES['agent_edit_logo']['name']) && $_FILES['agent_edit_logo']['name'] != '')
		{
			...
			$this->db->query("UPDATE agent SET name='$fname',company_name='$lname',mobile='$mo_no',office_phone='$ph_no',address='$address',city='$city',country='$country',postal_code='$post_code',currency_type='$c_type',agent_mode='$a_type',call_center_id='$cc_id' , agent_logo='$agent_logo' ,markets=' $market' WHERE agent_id='$id'");
		}
		else
		{
			$this->db->query("UPDATE agent SET name='$fname',company_name='$lname',mobile='$mo_no',office_phone='$ph_no',address='$address',city='$city',country='$country',postal_code='$post_code',currency_type='$c_type',agent_mode='$a_type',call_center_id='$cc_id' ,markets=' $market' WHERE agent_id='$id'");
		}
	...
	function sup_registration_page()
	{
		...
		else
		{
		   ...
		   $password = $this->input->post('password');
			...
			if($this->admin_model->add_supplier($name, $company_name, $address, $country, $city, $postal_code, $office_phone, $mobile_no,$currency, $email, $password, $hotel_logo,$markets)) 			{
				$config['protocol'] = 'smtp';
				$config['smtp_host'] = 'mail.provab.com';
				$config['smtp_port'] = 25;
