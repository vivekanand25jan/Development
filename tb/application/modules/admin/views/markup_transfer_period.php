...
<div style="margin-top:5px;">
    ...
    <form name="form1" action="<?php echo WEB_URL; ?>index/add_markup_transfer_period" method="post">
    
    
    <table width="780" border="0" cellspacing="1" cellpadding="0"><caption><h4 align="left">Transfer Period Information</h4></caption>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" style="background-color:#E8E5E5">
  ...
  <?php 
  if($excursion_markup[0]!='')
  {
for($i=0;$i<count($excursion_markup);$i++)
{
	
	?>
    <?php 
			$fd = $excursion_markup[$i]->from_date ; 
			$fds = explode("-",$fd); 
			$from_date = $fds[2].'-'.$fds[1].'-'.$fds[0]; 
			$td = $excursion_markup[$i]->to_date ; 
			$tds = explode("-",$td);
   ...
   <tr >
    ...
    <td align="center" class="admin-table-colo1"><a href="<?php echo WEB_URL; ?>index/edit_tr_markup_period/<?php echo $excursion_markup[$i]->transfer_markup_period_id; ?>" style="text-decoration:none;color:#099; "><?php echo $from_date .'-' .$to_date; ?></a></td>
      <td align="center" class="admin-table-colo1"><a href="<?php echo WEB_URL; ?>index/view_tr_markup_details/<?php echo $excursion_markup[$i]->transfer_markup_period_id; ?>" style="text-decoration:none;color:#099; ">Add Markup</a></td>
    <td align="center" class="admin-table-colo1"><?php if($excursion_markup[$i]->status==1) { echo 'Active'; } else { echo 'InActive'; } ?></td>
    <td align="center" class="admin-table-colo1">
    <a href="<?php echo WEB_URL; ?>index/update_tr_markup_period/<?php echo $excursion_markup[$i]->transfer_markup_period_id; ?>/1 ">Active</a> / 
    <a href="<?php echo WEB_URL; ?>index/update_tr_markup_period/<?php echo $excursion_markup[$i]->transfer_markup_period_id; ?>/0">InActive</a>/ 
    <a href="<?php echo WEB_URL; ?>index/update_tr_markup_period/<?php echo $excursion_markup[$i]->transfer_markup_period_id; ?>/2">Delete</a></td>
  </tr>
 <?php
}
  }
