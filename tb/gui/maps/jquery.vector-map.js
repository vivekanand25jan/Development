	...
	VectorCanvas.prototype = {
        ...
        applyTransformParams: function(scale, transX, transY) {
            if (this.mode == 'svg') {
				...
				//find active map to reset the transform
				var mapItem = $('#airportsByMap .mapMenu li.active').attr('rel')
				
				
				
				for(var i=0;cordArr.maps.length>i;i++)
				{
					if(mapItem==cordArr.maps[i].reg)
					{
						var m = '.'+cordArr.maps[i].reg+'.map' 	
						this.rootGroup.setAttribute('transform', cordArr.maps[i].cord);	
						return;
					}
				}	
				//stjook ends
