<table width="100%" border="0" style="margin-top:20px" cellspacing="7" cellpadding="3" class="r-hoteldeta">
		  ...
		  <tr>
                    ...
                    <td>: <?php echo $trans->agentvouchercode ; ?></td>
                  </tr>
                </table>
                </td>
          </tr>
  ...
  <tr>
	  <?php
		if($trans->status!="Cancelled" && $trans->cancelled_charge==''){
			$ext_pri = 0;
			$extra = explode(",",$result_view->extra_services);  
	for($nth=0;$nth<count($extra);$nth++){ $cc = explode('--',$extra[$nth]);$ext_pric[]=$cc[1];$ext_pri+=$cc[1];} echo $ext_pri;
    ?>
    <td>Total Fare :<?php echo $trans->xml_currency; ?> <?php echo $trans->amount; ?></td>
    <?php } else{ echo $ext_pri;?>
    <td>Total Fare :<?php echo $trans->xml_currency; ?> <?php echo ($trans->check_per!=420)?$trans->cancelled_charge:$trans->cancelled_charge+$ext_pri; ?></td>
    <?php } ?>
     </tr>
</table>
...
-->
	...
	<?php
		...
		for($c=0;$c< count($canPolicies);$c++)
		{
			...
			echo "<div>If you cancel this <font color='red'><b>on or after</b></font> the date ".$canTill[0].", ".$trans->xml_currency." ".$canTill[1]*$trans->currency_val." will be the charge</div>";
		}
	?>
	</td>
     <td style="border-bottom: 1px solid #9D9D9D;border-right: 1px solid #9D9D9D;">
	 <?php
      //echo ($trans->amount-$trans->cancellation_till_charge);
		...
		{
			$canTill = explode("___",$canPolicies[$c]);
			$Am = $canTill[1]*$trans->currency_val.'/'.$trans->amount;
			$tam = $trans->amount;
			$refundCharge = $tam-$Am;
			echo "<div>".$trans->xml_currency." ".$refundCharge."</div>";
		}
     ?></td>
