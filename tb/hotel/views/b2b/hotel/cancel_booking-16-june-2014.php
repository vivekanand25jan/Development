...
-->
  ...
  <tr><td><font size="+1"  color="#333333">Cancellation Policy</font></td></tr>
   <tr><td><font size="+0.8" color="#333333">Cancellation Charge applicable:  
	...
	{
		$cancels[$i];
		$cancelPolicies = explode("//", $cancels[$i]); 
//echo count($cancelPolicies);
//echo "<pre/>";print_r($cancelPolicies);
$totCancelAmt=0;
		for($j=0; $j< count($cancelPolicies); $j++)
		{
			$cancelPolicies[$j];
			$noOfCancelPolicies = explode("___", $cancelPolicies[$j]);
			$cancel_date = $noOfCancelPolicies[0];
			$cancel_Amt  = $noOfCancelPolicies[1];
$date_curr=strtotime(date('Y-m-d'));
$date_cancel=strtotime($cancel_date);
			$date_cu=date("Y-m-d",$date_curr);
			$date_ca=date("Y-m-d",$date_cancel);
			//echo    '<br/>';
			if($date_cu >= $date_ca)
			{
				$cancel_date = $noOfCancelPolicies[0];
		...
		$totCancelAmt = $totCancelAmt + $cancel_Amt;
		}
		//echo    '<br/>';
		
	}
	echo 'AED '.$totCancelAmt;
	?>
   
   </font>
   </td></tr>

   <tr><td><input type="hidden" name="totCancelAmt" value="<?php echo $totCancelAmt; ?>"></td></tr>
