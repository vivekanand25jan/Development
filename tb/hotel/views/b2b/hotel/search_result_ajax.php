...
if($result != '')
{
	...
	{	
		...
		$first_date = $room_alloc_date;
				...
				//echo $_SESSION['ed']//;$end_date = explode('-',$_SESSION['ed']);
		//$room_end_date = $end_date[2].'-'.$end_date[1].'-'.$end_date[0];
		//$last_date = $room_end_date;
					//$ldate = $last_date; 
					//$ld = explode("-", $ldate); 		
				//echo $priceEDate = date("Y-m-d", mktime(0,0,0,$ld[1], $ld[2] + $ld, $ld[0]));

		$sql="SELECT DISTINCT(allocation_rooms),allocation_release_days,allocation_validity_from,allocation_validity_to,hotel_id FROM sup_inv_cate_allotment WHERE hotel_id='".$result[$i]->hotel_id."' AND cat_type='".$val1[0]."' AND season_id='".$season."' AND allocation_validity_date='".$priceDate."'";
		//echo $sql="SELECT DISTINCT(allocation_rooms) FROM sup_inv_cate_allotment WHERE hotel_id=16 AND cat_type='Standard'";
		$query=$this->db->query($sql);
		$res=$query->result();
		
		//print_r($res);
		$allot_value= $res[0]->allocation_rooms;
		$allocation_release_days=$res[0]->allocation_release_days;
                
               $allocation_validty_from = $res[0]->allocation_validity_from;
              ...
              $strtoprice = strtotime($priceDate)-(86400*($allocation_release_days-1));
                $pd = strtotime($_SESSION['ed']);
                 $ainstrtime = $strallocationfrom;
               //echo date("Y-m-d",$ainstrtime); echo "--";
                 
                 $alocto = strtotime($res[0]->allocation_validity_to);
...
<div class="hotelnames">
    ...
    <h2><a href="<?php echo WEB_URL.'b2b_hotel/hotel_detail/'.$result[$i]->api_temp_hotel_id; ?>"  target="_blank"><?php echo $result[$i]->hotel_name;//echo "<pre/>";print_r($result);exit; ?></a></h2>
    <div style="margin-top:-5px;"><?php $star = $result[$i]->star; 
											   ...
											   ?>   </div>
        <h2><?php echo $result[$i]->city;?> | <a href="<?php $hotel_id = explode('CRS',$result[$i]->hotel_code); print WEB_URL.'b2b_hotel/hotel_info/'.$hotel_id[1]; ?> " target="_blank" class="text3">Hotel Info</a> | <a href="<?php print WEB_URL.'b2b_hotel/mapping_fun/'.$result[$i]->api_temp_hotel_id; ?> "  onclick="$('#rlightbox').show();" target="rlight_box" class="text3">Show Map</a> | <a href="<?php echo WEB_URL.'b2b_hotel/hotel_detail/'.$result[$i]->api_temp_hotel_id; ?>"  target="_blank"  class="text3">View Rates & Book </a> | 
        <?php 
        if(!empty($allocation_release_days) && $allot_value > 0 && $strtoprice>=$ainstrtime && $alocto>=$pd) {?>
        <font color="green"> <?php echo "Available"; ?></font>
        <?php }else { ?>
        <font color="red"><?php echo "On Request"; ?></font>
        <?php }?></h2>
