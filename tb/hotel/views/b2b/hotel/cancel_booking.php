...
-->
  ...
  <tr><td><font size="+1"  color="#333333">Cancellation Policy</font></td></tr>
   <tr><td><font size="+0.8" color="#333333">Cancellation Charge applicable:  
	<?php
	$first = explode(',',$result_view->cancellation_till_charge);
	if(count($first)==1){
	$cur_val = $result_view->currency_val;
...
//}
	...
	$cur_val = $result_view->currency_val;
	$cancel_Amt=0;
	for($mk=0;$mk<count($first);$mk++){
	$cancels = explode("//", $first[$mk]);
