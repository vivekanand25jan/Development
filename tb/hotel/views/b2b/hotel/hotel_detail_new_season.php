...
{ 
	...
	//echo "<pre/>";print_r($hotel_category_types);
...
<table width="100%" border="0" cellspacing="10" cellpadding="0" class="hotelnames" style="margin-bottom:3px;border: 1px solid #BEE7FF;">
    ...
    <?php
     ...
     for($c=0;$c < count($hotel_category_types[0]);$c++) {  
	$hotel_season_prices = $this->B2b_Hotel_Model->get_hotel_season_prices($s_adult, $s_child,$hotel_code,$sec_res,$s_date,$e_date,$hotel_category_types[$c]->category_type);
    ...
    <td align="left" valign="top" width="700">
		...
		$room_vecate_date = $end_date[2].'-'.$end_date[1].'-'.$end_date[0];
				$start = strtotime($room_alloc_date);
					...
					$get_hotel_promotion_stay_pay = $this->B2b_Hotel_Model->get_hotel_promotion_stay_pay($start,$days,$room_alloc_date, $room_vecate_date, $hotel_id[1], $hotel_category_types[$c]->sup_rate_tactics_id); 
                ...
                <?php  
					...
					//echo "hiiiiiii";print_r($hotel_season_prices);
					 ...
					 //echo "hiiiiii";
					 //echo "<pre/>";print_r($get_hotel_daily_rates);
...
//print_r($get_hotel_daily_rates); die;
				//print_r($_SESSION['child_age']); echo $i ; die;
				if($i=='0'){
					for($rch=0; $rch<6; $rch++){
						if($_SESSION['child_age'][$rch]!='0'){
							if($_SESSION['child_age'][$rch] >= $get_hotel_daily_rates[0]->child_above_age && $_SESSION['child_age'][$rch] < $get_hotel_daily_rates[0]->child_below_age)
							{
				...
				/* EARLY BIRD WRITTEN BY VIVEK */
				 ...
				 $get_hotel_early_bird_rates = $this->B2b_Hotel_Model->get_hotel_early_bird_rates($room_alloc_date, $room_vecate_date, $hotel_id[1], $room_type, $season_id);
					...
					{  
						...
						$discount_amt = ($discount /100) * $get_hotel_daily_rates[0]->rate ;
						 $total_cost = $get_hotel_daily_rates[0]->rate - $discount_amt;
					...
					{	
						$total_cost = $get_hotel_daily_rates[0]->rate ;
						$total_costwm = $get_hotel_daily_rates[0]->rate ;
					}
					
					/* EARLYBIRD CODE END WRITTEN BY VIVEK*/
					$splChar = $get_hotel_daily_rates[0]->supplementary_charge_rate + $c_child_above_age_charge + $c_sec_child_charge;
					//echo $get_hotel_daily_rates[0]->rate; die;
					
					$amountv1 = $roomPlusSupplementCharge=$total_cost + $splChar;
				...
				//echo "start".$start;
				//echo $checkfornight."night";
			//echo $prom_from=$get_hotel_promotion_stay_pay[0][0]->promotion_avail_date_from;
				 //echo "end".$end
			//echo $prom_to=$get_hotel_promotion_stay_pay[0][0]->promotion_avail_date_to;
...
$kk = $this->session->userdata('curr_values')*$total_cost;
		...
		$first_date = $room_alloc_date;
                ...
                //print_r($get_hotel_daily_allotment); die;
                
              $allocation_validty_from = $get_hotel_daily_allotment[0][0]->allocation_validity_from;
              //echo $get_hotel_daily_allotment[0][0]->allocation_validity_to;
              //echo $priceDate; 
               ...
               $strtoprice = strtotime($priceDate)-(86400*($allocation_release_days-1));
                //$strtoprice = strtotime($priceDate);
                 $ainstrtime = $strallocationfrom;
               //echo date("Y-m-d",$ainstrtime); echo "--";
              // echo $strtoprice; echo "--";echo $ainstrtime;
		...
		//echo "<pre/>";print_r($get_hotel_promotion_stay_pay );
			//echo "stay nights".$get_hotel_promotion_stay_pay[0][0]->stay_nights;
			if(!empty($get_hotel_promotion_stay_pay) && $counterdays >= $get_hotel_promotion_stay_pay[0][0]->stay_nights){ //echo "hi";
				if(($get_hotel_promotion_stay_pay[0][0]->stay_nights!='' && $counterdays >= $get_hotel_promotion_stay_pay[0][0]->stay_nights && $get_hotel_promotion_stay_pay[0][0]->stay_nights!='0' && $stay>0) || $get_hotel_promotion_stay_pay[0][0]->bb_date!='0000-00-00'){ //echo "hello";
			?>
                ...
                <?php } ?>
                <?php if($get_hotel_promotion_stay_pay[0][0]->stay_nights!='' && $counterdays >= $get_hotel_promotion_stay_pay[0][0]->stay_nights && $get_hotel_promotion_stay_pay[0][0]->stay_nights!='0' && $stay>0) { ?>
				<div style="color:#517194; font-size:12px;line-height: 15px;">STAY <?php echo $get_hotel_promotion_stay_pay[0][0]->stay_nights; ?> PAY <?php echo $get_hotel_promotion_stay_pay[0][0]->pay_nights; ?> PROMO - Stay <?php echo $get_hotel_promotion_stay_pay[0][0]->stay_nights; ?> Pay for <?php echo $get_hotel_promotion_stay_pay[0][0]->pay_nights; ?></div>
                
                <?php } if($get_hotel_promotion_stay_pay[0][0]->bb_date!='0000-00-00' ){ ?>
                <div style="color:#517194; font-size:12px;line-height: 15px;">BOOK BB 
				<?php //$bb_date = explode("-", $get_hotel_promotion_stay_pay[0][0]->bb_date); echo $bb_date[2].'-'.$bb_date[1].'-'.$bb_date[0]; ?> UPGRADE TO HB 
				<?php //$hh_date = explode("-", $get_hotel_promotion_stay_pay[0][0]->hh_date); echo $hh_date[2].'-'.$hh_date[1].'-'.$hh_date[0]; ?></div>
                <?php } ?>
				</div>
			<?php }
			?>
            ...
            <td align="center" valign="top" width="155" bgcolor="#eee" style="color:#966693; font-size:14px;"><?php echo $hotel_season_prices[$sp]->xml_currency;?> <?php echo number_format((float)$grand_total_cost*$this->session->userdata('curr_values'), 2, '.', ''); //$hotel_season_prices[$sp]->total_cost;?> <br />
            <?php
			...
			$ssdd = strtotime($_SESSION['sd']);
			$eedd = strtotime($_SESSION['ed']);
			
			
			//echo $hotel_minimum_stay[0]->minimum_stay_nights;
			if(isset($hotel_minimum_stay[0]->minimum_stay_nights) && $hotel_minimum_stay[0]->minimum_stay_nights!='' && $min_stay_from <= $ssdd && $min_stay_to >= $eedd){   
				if($hotel_minimum_stay[0]->minimum_stay_nights <= $days){
					//$_SESSION['minstay']='false'; 
					$minstay=0;
			?> 
            
            <input type="radio" name="room_<?php echo $i;?>" id="room" value="<?php echo $grand_total_cost*$this->session->userdata('curr_values'); ?>,<?php echo $hotel_season_prices[$sp]->api_temp_hotel_id; ?>,<?php echo $org_costs; ?>,<?php echo $markups; ?>,<?php echo $hotel_season_prices[$sp]->sup_apart_maintain_month_id; ?>,<?php echo $hotel_season_prices[$sp]->room_code; ?>,<?php echo $hotel_season_prices[$sp]->market_id; ?>,<?php echo $hotel_season_prices[$sp]->room_type; ?>,<?php echo $hotel_season_prices[$sp]->category_type; ?>,<?php echo $hotel_season_prices[$sp]->season; ?>" class="radiobutton"/><br />
            ...
            <input type="hidden" name="grand_total_cost[]" id="grand_total_cost" value="<?php echo $grand_total_cost*$this->session->userdata('curr_values'); ?>" />
            </td>
        </tr>
        ...
        <tr class='classic_room' width="700">
            ...
            <?php //echo "hello";echo "<pre/>";print_r($hotel_room_cancel_policy);
            //print_r($_SESSION);

            
            $bodate = strtotime($_SESSION['sd']);
            //exit;
            	for($hcp=0; $hcp< count($hotel_room_cancel_policy); $hcp++)
            	{	
            		$bodatec = strtotime($_SESSION['sd'])+($hotel_room_cancel_policy[$hcp]->cancel_policy_nights*24*60*60);
            		//echo "hiiiiiii";
			...
			?>	
					<?php
						if($bodatec>$bodate){
					?>
            		A charge of <?php echo $this->session->userdata('currency_type').' ';?> <?php echo number_format((float)$per_all_nights_cancel_rate*$this->session->userdata('curr_values'), 2, '.', ''); ?> will apply if Cancelled or Amended for <?php echo $hotel_room_cancel_policy[$hcp]->cancel_policy_nights; ?> days prior to arrival.<br />
            		<?php }else{ ?>
            		A charge of <?php echo $this->session->userdata('currency_type').' ';?> <?php echo number_format((float)$per_all_nights_cancel_rate*$this->session->userdata('curr_values'), 2, '.', ''); ?> will apply if Cancelled or Amended for <?php echo $hotel_room_cancel_policy[$hcp]->cancel_policy_nights; ?> days prior to arrival.<br />
            		<?php } ?>
    ...
    <tr>
        ...
        $start = strtotime($room_alloc_date);
					...
					$days = $end - $start;
					 ...
					 $ssddd = strtotime($_SESSION['sd']);
			$eeddd = strtotime($_SESSION['ed']);
			
			
if(isset($hotel_minimum_stay[0]->minimum_stay_nights) && $hotel_minimum_stay[0]->minimum_stay_nights!='' && $min_stay_from <= $ssddd && $min_stay_to >= $eeddd){ 
...
$('.classic_room').click(function(){
	...
	//alert(totalAmt);
	if(totalAmt!='NaN'){
	$('#resultTotal').html("<?php echo $this->session->userdata('currency_type').' ';?> "+totalAmt); }
