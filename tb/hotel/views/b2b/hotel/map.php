...
var projmark = JSON.parse($.ajax({
     type: 'GET',
     url: '<?php echo WEB_URL.'hotel/fetch_search_result_map_select/'.$result_id; ?>',
     dataType: 'json',
     global: false,
     async:false,
...
google.maps.event.addDomListener(window, 'load', initialize);
   ...
   <strong style="color:#3399FE; font-size:14px; padding:6px 0 0 15px;">        
        <img  width="37" height="43"src="<?php echo WEB_DIR.'/images/select.png' ?>"  style="float:left;"/> <span style="float:left; margin:3px 0 0 10px; font-size:14px; font-family:Arial, Helvetica, sans-serif;"><?php echo $result[0]['hotel_name']; ?> </span>   
   <?php   
...
<div align="right"  style=" margin:0 0 0 15px"><a class="ac_det" href="javascript:void(0);" onClick="parent.$('#rlightbox').attr('src','');parent.$('#rlightbox').hide();"><input type="button"  value="Go Back"  style="-moz-border-radius:4px;
	...
	" /></a></div>
 	<input type="button" name="other" value="Hide Other Hotels" id="other_hotel" class="hide" onclick="roam('name');" style="-moz-border-radius:4px;
	-webkit-border-radius:4px;
	...
	background-position: 0 0;
	" />
	<img src="<?php echo WEB_DIR.'/images/unselect.png' ?>"  style="float:left; margin-left:-39px"/>
