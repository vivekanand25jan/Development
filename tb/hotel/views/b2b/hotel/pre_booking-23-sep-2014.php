...
$kk = $this->session->userdata('curr_values')*$total_cost;
				...
				$h++;
					...
					echo $grand_total_costwm = ($grand_total_costwm+$total_costwm).'vivek';
	    ...
	    $cancelPolicyPlan.='<br />'; } 
            	for($hcp=0; $hcp< count($hotel_room_cancel_policy); $hcp++)
            	{
					...
					{	//echo "welcome";
						...
						echo $cancel_ratewm = ($hotel_room_cancel_policy[$hcp]->cancel_policy_percent/100) * $grand_total_costwm;		
						$dailyRatesArray=$cancelsAmt;  
						$dailyRatesArraywm=$cancelsAmtwm;  
						if($cancel_rate=='0')
						{
							echo $per_all_nights_cancel_rate = $cancel_rate;
							$per_all_nights_cancel_ratewm = $cancel_ratewm;
						}
						else if($cancel_rate<$dailyRatesArray[0])
						{
							echo $per_all_nights_cancel_rate = $dailyRatesArray[0];
