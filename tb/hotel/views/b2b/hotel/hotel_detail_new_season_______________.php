    ...
    <td align="left" valign="top" width="700">
        ...
        <tr class='classic_room'>
            ...
            <td align='left' valign='middle' colspan="4" style="line-height:35px;font-size:13px;">
				<?php 
					...
					$get_hotel_promotion_stay_pay = $this->B2b_Hotel_Model->get_hotel_promotion_stay_pay($room_alloc_date, $room_vecate_date, $hotel_id[1], $hotel_category_types[$c]->sup_rate_tactics_id); 
				...
				//echo $get_hotel_daily_rates[0]->meal_plan;
				...
				<?php 
					...
					if(!empty($get_hotel_promotion_stay_pay) && $stay_nights!='0' && !empty($pay_nights) && $pay_nights < $pay_count)
					{
			...
			<?php //print_r($get_hotel_promotion_stay_pay);
			if(!empty($get_hotel_promotion_stay_pay)){ 
			...
			if(isset($hotel_minimum_stay[0]->minimum_stay_nights) && $hotel_minimum_stay[0]->minimum_stay_nights!='' && $min_stay_from <= strtotime($room_alloc_date) && $min_stay_to >= strtotime($room_vecate_date)){  
            ...
            <?php } else { echo '<span style="color:red;font-weight:normal;">Minimum Stay Nights: '.$hotel_minimum_stay[0]->minimum_stay_nights.'</span><br />'; } }
			else { ?>
             <input type="radio" name="room_<?php echo $i;?>" id="room" value="<?php echo $grand_total_cost; ?>,<?php echo $hotel_season_prices[$sp]->api_temp_hotel_id; ?>,<?php echo $org_costs; ?>,<?php echo $markups; ?>,<?php echo $hotel_season_prices[$sp]->sup_apart_maintain_month_id; ?>,<?php echo $hotel_season_prices[$sp]->room_code; ?>,<?php echo $hotel_season_prices[$sp]->market_id; ?>,<?php echo $hotel_season_prices[$sp]->room_type; ?>,<?php echo $hotel_season_prices[$sp]->category_type; ?>,<?php echo $hotel_season_prices[$sp]->season; ?>" class="radiobutton"/><br />
             <?php } ?>
        ...
        <tr class='classic_room' width="700">
            ...
            <?php /*if(isset($hotel_room_cancel_policy[0]->cancel_policy_from) && $hotel_room_cancel_policy[0]->cancel_policy_from!=''){
				$cancel_policy_from = explode('-',$hotel_room_cancel_policy[0]->cancel_policy_from);
				$cancel_policy_from = $cancel_policy_from[2].'-'.$cancel_policy_from[1].'-'.$cancel_policy_from[0];
				$deadline_date = date('Y-m-d', strtotime('-'.$hotel_room_cancel_policy[0]->cancel_policy_nights.' days', strtotime($cancel_policy_from)));
           ...
           <!-- <div style='color:red;font-weight:normal;'>WITH IN CANCELLATION DEADLINE</div>--> <?php //}} ?>
            ...
            <?php /*if(isset($hotel_room_cancel_policy->cancel_policy_from) && $hotel_room_cancel_policy->cancel_policy_from!=''){ 
				...
				if($hotel_room_cancel_policy->cancel_policy_percent!='' || $hotel_room_cancel_policy->cancel_policy_percent!='0')
				{
					$per_cent = $hotel_room_cancel_policy->cancel_policy_percent / 100;
					$per_night_total = $per_cent * $grand_total_cost;
					$per_all_nights_cancel_rate = $hotel_room_cancel_policy->cancel_policy_nights * $per_night_total;
				}
				else if($hotel_room_cancel_policy->cancel_policy_charge!='' || $hotel_room_cancel_policy->cancel_policy_charge!='0')
				{
					$per_all_nights_cancel_rate = $hotel_room_cancel_policy->cancel_policy_nights * $hotel_room_cancel_policy->cancel_policy_charge;
				}*/
			?>
            <!--A charge of AED <?php echo $per_all_nights_cancel_rate; ?> will apply if Cancelled or Amended between <?php echo $hotel_room_cancel_policy->cancel_policy_from; ?> and <?php echo $hotel_room_cancel_policy->cancel_policy_to; ?><br />--><?php //} ?>
