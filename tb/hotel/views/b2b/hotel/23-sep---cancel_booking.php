...
<?php  
$extra_pri = $this->B2b_Hotel_Model->getExtra($result_view->hotel_booking_id);

$exp = explode(',',$extra_pri);
if(count($exp)>1){
    $ex_pri = 0;
    for($n=0;$n<count($exp);$n++){
    $aex = explode('--',$exp[$n]);
    $ex_pri+= $aex[1]; 
    }
}else{
    $aex = explode('--',$extra_pri);
    $ex_pri = $aex[1];
}
?>
...
$cancel_Amt = ($m[$index]*$result_view->currency_val!=0)?($m[$index]*$result_view->currency_val)+$ex_pri:$m[$index]*$result_view->currency_val;
$cancel_Amts = ($ff[$index]*$result_view->currency_val!=0)?($ff[$index]*$result_view->currency_val)+$ex_pri:$ff[$index]*$result_view->currency_val;
...
echo $cancel_Amt;
   ...
   <tr><td><input type="hidden" name="totCancelAmta" value="<?php echo $cancel_Amts; ?>">
