...
on sup_apart_maintain_month.hotel_id = abc.hotel_id) mm 
		...
		if ($query->num_rows() > 0)
		{
		  ...
		  //echo "<pre/>"; print_r($datas);
		   ...
		   {
			  //$currencyv1='AED';
			  $currencyv1=$this->session->userdata('currency_type');
			  ...
			  $api="crs";
				...
				$sec_res=$_SESSION['ses_id'];
					if($currencyv1 !='AED')
					{
						...
						$c_val = $c_val1->value;
						$this->session->set_userdata('curr_values',$c_val);
...
$xml_data = '<availabilityRQ xmlns="http://www.hotelbeds.com/schemas/messages" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	<stay checkIn="'.date("Y-m-d",strtotime($sd)).'" checkOut="'.date("Y-m-d",strtotime($ed)).'"/>
	<occupancies>
...
//echo $xml_data; die;
$apiKey = "da8ba0d1303ea39e6ae164c0af54c2f7";
$Secret = "695df0f9d1";



//$apiKey = "a9d29f1c46c813465b1ef7a78f3384b1";
//$Secret = "01fa192962";
...
if($dbc_room_cnt==1)
		...
		{
		$refcode=date("YmdHis");
   		/*$xml_data ='<availabilityRQ echoToken="DummyEchoToken" sessionId="'.$refcode.'" xmlns="http://www.hotelbeds.com/schemas/messages" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	<Language>ENG</Language>
	<Credentials>
	<User>HTLEXPLORAE64882</User>
    <Password>HTLEXPLORAE64882</Password>
...
//echo $output;exit;
	...
	{ 
		...
		$h_id = $result_view->hotel_booking_id;
		$booking_amount = $result_view->amount;
...
//echo $output;exit;
		...
		{  
			...
			$totCancelAmtAdmin = $this->input->post('totCancelAmtAdmin');
			$totCancelAmtad = $this->input->post('totCancelAmta');
			$cancel_for_dates = $this->input->post('cancel_for_dates');
			$check_per = $this->input->post('checkforper');
			$reminder_amount = $booking_amount-$totCancelAmt;
			$statusval='Cancelled';
			...
			if($statusval != '') 
			{
				...
				//	$this->B2b_Hotel_Model->debitBookingAmount_credit($agent_id,$branch_id,$red_amount);
				//}
				if($result_view->status!='Pending for Confirmation'){
					$this->B2b_Hotel_Model->debitBookingAmount_credit($agent_id,$branch_id,$reminder_amount);
				}
				$data = array(
					'status' => $statusval,
					'cancelled_charge' => $totCancelAmt,
					'cancel_amount_admin' => $totCancelAmtAdmin,
					'check_per' => $check_per,
					'cancel_for_dates' => $cancel_for_dates
				);
...
$checkouzt='';
	...
	$earlyBirdPer = (isset($_SESSION['earlyBirdPer']))?$_SESSION['earlyBirdPer']:'';
	$earlyBirdAmt = (isset($_SESSION['earlyBirdAmt']))?$_SESSION['earlyBirdAmt']:'';
		$data5 = array(
		 'product_id' => 2,
		 'user_id' => $agent_id,
		 ...
		 'created_date' => date("Y-m-d"),
		   'user_type' => $agent_type,
			...
			'daily_rates'=> $value['room_daily_rate'],
			'agentvouchercode' =>$value['agtvoucher'],
             'extra_service_admin'=>$value['tServicesw'],
			 'agent_mode' => $value['agent_mode'],
			 'earlyBirdPer'=>$earlyBirdPer,
			 'earlyBirdAmt'=>$earlyBirdAmt
...
if($_SESSION['book_final_book_val']['agent_mode']!='cash')
{
    ...
    $this->B2b_Hotel_Model->inser_customer_book_hotelpro_trans_hotel($trans_id,$ConfirmationNumbervalue,$user_id,$val_last);
	 ...
	 else if($service->api == 'crs')
	 {
		...
		if($api=='crs')
		{
			...
			$with_admin = $value['org_amt']/$value['currency_val'];
			$with_admin_markup = (($markup_org /100) * $with_admin)+$with_admin;
			if($pending_confi!='vivek' && $pending_confi==''){				
	...
	//==============================================
	//echo $trans->status; die;
	$subb='';
	if($trans->status=="Cancelled"){
		$heading = "<strong><u>CANCELLATION</u></span>";
		$subb.="<span><strong>Please cancel the booking and<br/>acknowledge the same</span><br/>";
	}else{
		$heading = 'BOOKING REQUEST';
		$subb.="<span><strong>Attn: Room Reservations</strong></span><br/>";
		$subb.="<span>Please book and confirm the hotel booking as follows :</span><br/>";
	}
			$to  = $contact_info->email ; // note the comma
			//$to  = "vivekanand.provab@gmail.com" ;
...
$msg='';
	...
	<table width="100%" border="0" cellspacing="7" cellpadding="3" class="r-hoteldeta">
	  ...
	  <td  class="right-hotel-name">
		...
		<td  class="right-hotel-name">
	   <span  style=" font-weight:bold;font-size:20px;"> <font color="#227fb0">'.$heading.'</font></span>
		</td>
	  </tr>
	  <tr>
	  <td colspan="2">
	...
	<span style="float:left;">Hotel Name : '.$result_view->hotel_name .'</span>
	  ...
	  <tr>
		<td colspan="2" ><span style="font-size:15px">'.$subb.'</td>
	  </tr>';
	  ...
	  $msg.=' <tr>
		<td width="350" align="left" valign="top"><table width="100%" border="0" cellspacing="5" cellpadding="5" class="sum-txt">
			  ...
			  <tr>
				...
				<td><strong>: '.$child_count.' Children</strong></td>
			  </tr>
			  <tr>
				<td><strong>Voucher Date</strong></td>
				<td><strong>: '.$trans->created_date.'</strong></td>
  ...
  <div style="width:700px; margin:0 auto;">';
    ...
    <table align="left" width="724" border="0" cellspacing="0" cellpadding="5" class="sum-txt" style="margin-top:2px; font-size:15px; border:1px #bee7ff solid; border-radius:10px;">
      <tr>
         <td align="left">
          <div style="width:700px; margin:0 auto;">
            <div style="width:150px; height:84px; padding:10px; border:1px solid #333;float:left; font-size:24px; font-family:Times New Roman, Times, serif; "> Payment </div>
