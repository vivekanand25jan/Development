...
echo $i."<br>";
	...
	public function fetch_search_result_filter() 
	{
		...
		$tmp_data = $this->Hotel_Model->fetch_search_result($_SESSION['ses_id'], 1, $minVal, $maxVal, $minStar, $maxStar);
...
//print_r($data);exit;
	...
	function all_filter_new1($id)
	{
			...
			$data = $this->Hotel_Model->fetch_search_result_for_page2($_SESSION['ses_id'],$id, $minVal, $maxVal, $minStar, $maxStar);
...
//print_r($_SESSION); exit;
 $hotellisst=$this->Hotel_Model->fetch_search_result_for_page($_SESSION['ses_id'], $page, $minVal, $maxVal, $minStar, $maxStar);
  ...
  if(isset($parser->document->responsedetails[0]->searchhotelpriceresponse[0]->hoteldetails[0]->hotel))
  {
	...
	function ajax_map_show($result_id)
	{
		...
		//echo "sss";echo'<pre/>';print_r($hot_lat_long);exit;
			...
			$page_hot_lat_long=$this->Hotel_Model->get_searchresult_all($lat1,$long1,$hotel_name1,$city1);
...
$fav_hotel_detail=array();
	...
	function pre_booking()
	{
		...
		if($room_count == 1)
		{
			...
			$data['service']=$service;
			$sec_res=$_SESSION['ses_id'];
			$data['result_id']=$_POST['result_id'];
			$rm_info=array();
		...
		{
			$result_id = explode("-",$_POST['result_id']);
			$service=$this->Hotel_Model->get_searchresult($result_id[0]);
			$api = $service->api;
			$hotel_code = $service->hotel_code;
			$hotel_name = $service->hotel_name;
			$star = $service->star;
			...
			$data['result_id']=$_POST['result_id'];
			$rm_info=array();
			for($r=0;$r< count($result_id);$r++)
			{
			 $rm_info[]=$this->Hotel_Model->fetch_gta_temp_result_room_result_id($sec_res,$result_id[$r]);
			}
		...
		$mer_id=explode("-",$_POST['result_id']);
...
<ExtraBeds>
	...
	// $guest_name = array('test'); 
		 for($i=0; $i< count($fname); $i++)
	 {
		...
		$data['city_co']=$_POST['city'];
		$data['mobile_co']='';
		$data['phone_co']=$_POST['mobile'];
		$data['email_co']=$_POST['email'];
		//echo '<pre>'; //print_r($data); exit;
	...
	$hotel_id = $service->hotel_code;
	 $roomcat=$service->room_code;
     $roomty=$service->room_type;
	 ...
	 if($service->api == 'hotelsbed')
	 {
		...
		$data['guestchild']=$guestchild=array_sum($child);
		
			$address=$contact_info->city;
			$remarks = '';
			$booked_amount_gta=$trans->amount;
			$cancel_amount=0;
			$canceldate='';
			
			$criteria=$_SESSION['adult_count'];		 
...
//print_r($_SESSION);exit;
	...
	//echo $data;
		...
		//echo $xmls;exit;
			...
			//$this->session->set_userdata($arr);
              ...
              $hotel=$dom->getElementsByTagName("HotelInfo");
			  foreach($hotel as $hotelName)
			  {
				$hotelval=$hotelName->getElementsByTagName("Name");	  
				$hotelNameval = $hotelval->item(0)->nodeValue;
				$hotelcode=$hotelName->getElementsByTagName("Code");	  
			...
			foreach($bookingItemCode as $ddd)
			{
            $bookingItemCodeval = $bookingItemCode->item(0)->nodeValue;
			}
           // echo $IncomingOfficecode.'-'.$bookingItemCodeval;
       ...
       $BoardTypeVal = $Board->item(0)->nodeValue;	 
		...
		$cancel = $dom->getElementsByTagName( "CancellationPolicy" );
		  ...
		  foreach($cancel as $canval)
		  {
		   ...
		   $dateFromTimeValc=$dateFromc->item(0)->getAttribute("time");
		   ...
		   $dateToValc=$dateToc->item(0)->getAttribute("date");
		    ...
		    $curr=$dom->getElementsByTagName( "Currency" );
		   $currencyCode=$curr->item(0)->getAttribute("code");	
		   
		   $datefrom= $dateFromValc;
		   $time= $dateFromTimeValc;
		   $dateto=$dateToValc;	
		   $year1=substr($dateto,0,4); $mon1=substr($dateto,4,2); $date1=substr($dateto,6,2);	   
		   
		   $cdsc=$canceldisplayValc.' '. $currencyCode;	   
                  
            }
			$year=substr($datefrom,0,4); $mon=substr($datefrom,4,2); $date=substr($datefrom,6,2); $hour=substr($time,0,2); $min=substr($time,2,2); 
			$cancelpol= 'cancellation made  from'.' '.$date.'/'.$mon.'/'.$year.', '.$hour.':'.$min.' '.'will be charged'.$canceldisplayValc.' '.$currencyCode;
			
			$cfd=$date.'-'.$mon.'-'.$year;
			$ced=$date1.'-'.$mon1.'-'.$year1;
		...
		$roomtypedesc='';
									$HotelRoom = $dom->getElementsByTagName( "AvailableRoom" );
							  ...
							  foreach($HotelRoom as $HotelRoom1)
							  {
									...
									$roomtypedesc .= $RoomCountval.' x '.$roomv1.'<br>';
	...
	//	$agent_balane=$this->Search_Model->update_balance($agentid,$cdsc);
...
if(isset($_SESSION['user_logged']))
	...
	$val_last=$this->Hotel_Model->inser_customer_book_hotelpro($bookingItemCodeval,$bookingItemCodeval,$statusval,$hotelcodeval,$dateFromValc,$dateToValc,$dateFromValc,$amount,$BoardTypeVal,$bookingItemCodeval,$amount,$amount,'hotelsbed',$amount,$api_uni_id,0,$email,'test',$roomcountss,$guestadult,$guestchild,$RoomTypeVal,$cancelpol,1,$amount,$trans_id);
			...
			$bookingItemCode=$dom->getElementsByTagName("BookingItem");
						...
						foreach($bookingItemCode as $cnno){
                           ...
                           $statusval = $status->item(0)->nodeValue;
						      ...
						      $ItemPrice_attr=$cnno->getElementsByTagName("ItemPrice_attr");

							  $HotelItem=$cnno->getElementsByTagName("HotelItem");
							  foreach($HotelItem as $hotdet)
							  {

								    $HotelRooms=$hotdet->getElementsByTagName("HotelRooms");
							     foreach($HotelRooms as $hotdet1)
								 {


								    $HotelRoom=$hotdet1->getElementsByTagName("HotelRoom");
							     foreach($HotelRoom as $hotdet2)
								 {

								  $Description=$hotdet2->getElementsByTagName("Description");
                           $Descriptionval = $Description->item(0)->nodeValue;
								 		...
								 		$meals = $hotdet->getElementsByTagName("Meals");
		$Breakfastval='';
		foreach($meals as $ee)
		{
		$Basis = $ee->getElementsByTagName("Basis");
		$Basisval=$Basis->item(0)->nodeValue;
		$a=$ee->getElementsByTagName("Breakfast");


							  foreach($a as $d2)
							  {
							 $Breakfastval=$a->item(0)->nodeValue;
							  }




		$BoardTypeVal=$Basisval." ".$Breakfastval;
		}
...
$bookingItemCodeval=$bookingItemCodeval;	
if(isset($_SESSION['user_logged']))
	{
	$user_id = 	$_SESSION['b2c_userid'];
	}
	...
	function voucher_email($id)
{
	 ...
	 $con_id = $result_view->customer_contact_details_id;
	  ...
	  $contact_info=$this->Hotel_Model->contact_info_detail($con_id);
...
<body style=" background-color: #E2DFDF;">
	...
	<div class="box" style="height:auto">
        ...
        <div class="box" >Hotel Name :'.$result_view->hotel_code .' <br />
       Check In : '.$result_view->check_in.' <br />
        Check Out : '.$result_view->check_out.'<br />
        </div>
        
        <div class="box">Voucher Date : '.$result_view->voucher_date.'
      <br />
        Adult :'. $result_view->adult.'<br />
        Child : '.$result_view->child.'
      
         </div>
         
         
           <div class="box">Booking No : '.$trans->booking_number.'
        <br />
        Room Type : '.$result_view->room_type.'<br />
        Inclusion :'.$result_view->inclusion.'
         </div>
         
         
        <div class="box">
       
        Cancellation Policy : '.$result_view->cancel_policy.'
        <br />
        Booking Amount : '.$trans->amount.' USD'.'<br />
        
        Room COunt : '.$result_view->no_of_room.'
        </div>
          <div class="box" style="height:auto;">
  ...
  for($i=0;$i< count($pass_info);$i++)
  {
    ...
    $msg .= '  Name : '.$pass_info[$i]->first_name.' '.$pass_info[$i]->last_name.'<br /> ';
  ...
  $msg .= ' </div>  </div>
	...
	function voucher($val_last)
	{
	 ...
	 $con_id = $data['result_view']->customer_contact_details_id;
	  ...
	  $data['contact_info']=$this->Hotel_Model->contact_info_detail($con_id);
	  $data['trans']=$this->Hotel_Model->transation_detail_contact($con_id);
	//$trans_id = $trans->transaction_details_id;
	
	// $con_id = $contact_info->customer_info_details_id;
	 $data['pass_info']=$this->Hotel_Model->pass_info_detail($con_id);
