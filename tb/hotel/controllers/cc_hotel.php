...
//error_reporting(0);
session_start();
class Cc_Hotel extends CI_Controller {
	...
	public function __construct()
	{
		...
		if(!isset($_SESSION['ses_id']))
		{
			...
			redirect('cc_hotel/index','refresh');
...
exit;
 ...
 $data=array('id'=>'visit','type'=>'visit','ip'=>'ss');
	...
	function ajax_left_price($starA,$starB)
	{
		...
		$this->load->view('cc/hotel/search_result_ajax',$data);
	}
...
public function backtosearch()
{
		...
		$this->load->view('cc/hotel/search_result');
...
function country_search($city)
{
	...
	$this->load->view('cc/hotel/pre_search',$data);
	}
	else
	{
		redirect('cc_hotel/index','refresh');
	...
	{
		$a_id =$_POST['agent'];
		$select = "select * from agent where agent_id = '$a_id' AND active = 1";
	...
	else
	{
		redirect('cc_hotel/index','refresh');
			...
			else
			{
				...
				$data['api_fs'] =$api_f;
				$this->load->view('cc/hotel/search_result',$data);
			}
		}
		else
		{
...
$this->session->set_userdata($newdata);
	...
	//	$hotel_search_result = $this->load->view('cc/hotel/search_result_ajax', $data, true);
		/*$paging = $this->load->view('cc/hotel/pagination', $data, true);
		$googlemap = $this->load->view('cc/hotel/googlemap', $data, true);
...
$count_fav='<a href="javascript:delete_fav('.$id.');"><img width="17" style=" position:relative; top:4px;"  src="'.WEB_DIR.'images/gray-close-delete-circle-icon.png"> Remove To My Favourite List ';
	...
	function fav_search($idf)
	{
		...
		redirect('cc/agent_page','refresh');
	}
...
$count_fav='<a href="javascript:add_fav('.$id.');"><img width="17" style=" position:relative; top:4px;"  src="'.WEB_DIR.'images/022-Heart-Icon.png"> Add To My Favourite List ';
	...
	function room_fac()
	{
		...
		//$data1['room_facility'] = $this->B2b_Hotel_Model->fetch_gta_temp_result_room_facility($_SESSION['ses_id']);
		 $this->load->view('cc/hotel/room_facility');
	}
	function hotel_fac()
	{
		
		//$data1['room_facility'] = $this->B2b_Hotel_Model->fetch_gta_temp_result_room_facility($_SESSION['ses_id']);
		 $this->load->view('cc/hotel/hotel_facility');
	}
	function chain_fac()
	{
		
		//$data1['room_facility'] = $this->B2b_Hotel_Model->fetch_gta_temp_result_room_facility($_SESSION['ses_id']);
		 $this->load->view('cc/hotel/chain_facility');
	}
...
//echo json_encode($data['result']);
	...
	function gta_table_london()
	{
		...
		if(!empty($data2))
		{
				...
				redirect('cc_hotel/yuvi_emailsuccess','refresh');
		}
...
//$URL ="http://xmlv5test2.travco.co.uk/trlink/link1/trlink";//test
	...
	function cancel_booking($id)
	{
		...
		$this->load->view('cc/hotel/cancel_booking',$data);
	}
...
//echo $output;exit;
			...
			$this->load->view("cc/hotel/cancellaion_end",$data);
...
echo json_encode($map_data);
	...
	function error_page($error)
	{
		...
		$data['error']=$error;
		echo "sss";
			$this->load->view('cc/hotel/error_page',$data);
	}
	...
	/*	if($api=='hotelsbed')
		{
			...
			$data['api'] = 'hotelsbed';
		$data['cur_id'] = $id;
		$this->load->view('cc/hotel/hotel_detail',$data);	
			}
	   elseif($api=='gta')
		{*/
...
//echo $result_id;exit;
 ...
 // echo '<pre/>';
  // print_r($data);exit;
					$this->load->view('cc/hotel/pro_pre_booking',$data);	
			}
    ...
    /*    if($currencyCode!="USD")
      ...
      $camt=number_format($finalValh,2,'.','');   */
       ...
       $data['cancel_policy']=$cancel_policy;
		...
		$this->load->view('cc/hotel/pre_booking',$data);
...
$travco_cancel_policy .=  ' After:'.$dateToTimeVal[0].' '.$dateFromVal[1].' '.$cancelDescVal[1].'will be charged'.'<br>';							
	...
	$this->load->view('cc/hotel/pre_booking',$data);	
		...
		else
		{
         ...
         redirect("cc_hotel/booking_final",'refresh');
		}
...
else
{
	...
	function pre_booking($result_id)
	{
		...
		if ($agent_type == 2) {
  		...
  		$data['balance_credit'] = $balance_credit;
 		 $cc_id = $this->session->userdata('cc_id');	
		$data['balance_credit_cc'] = $this->B2b_Model->getAgentAccInfo_cc_balance($cc_id);
   ...
   //  $policy->remarks;
	...
	function booking_roomtype($id)
	{
		...
		if($api=='hotelsbed')
		{
			...
			$data['result_id']=$id;
			$data['api'] = 'hotelsbed';
		$this->load->view('cc/hotel/pre_booking_view',$data);	
			}
	...
	$service=$this->B2b_Hotel_Model->get_searchresult($id);
		...
		$this->load->view('cc/hotel/booking_view',$data);	
	}
...
$with_admin_markup = (($markup_org /100) * $with_admin);
											...
											$agent_markup = $amountv2*$with_admin;
											//$agent_markup = $amountv3;
		...
		$data5 = array(
		 ...
		 'user_id' => $agent_id,
		  'agent_markup' => number_format($agent_markup,'2','.',''),
		   'user_type' => 6,
		   'callcenterid'=> $this->session->userdata('cc_id'),
		 'amount' => $value['amount'],
...
//	 $agent_id = $this->session->userdata('agent_id');
// $markup_a = $this->B2b_Model->get_markup($agent_id);
//$net = $valuea['amount']-$markup_a->markup;
	//$this->B2b_Hotel_Model->debitBookingAmount($agent_id,$branch_id,$net);

	$net = $value['amount']-number_format($agent_markup,'2','.','');
	//$this->booking_final_pay($id,$parent_transaction_id);
	
		redirect('cc_hotel/booking_final_pay/'.$id.'/'.$parent_transaction_id, 'refresh');
...
$user_id = 	$this->session->userdata('agent_id');	
 ...
 $this->B2b_Hotel_Model->inser_customer_book_hotelpro_trans_hotel($con_id_org,$bookingItemCodeval,$user_id,$val_last);
		$this->voucher_email($val_last);	
			...
			redirect('cc_hotel/voucher/'.$val_last, 'refresh');		
...
//$this->voucher($val_last);
	redirect('cc_hotel/voucher/'.$val_last, 'refresh');*/			
	 ...
	 {
		echo "This is LIVE. Dont Make Booking......";	//redirect('cc_hotel/voucher/'.$val_last, 'refresh');		
...
$this->B2b_Hotel_Model->debitBookingAmount_cc($agent_id,$branch_id,$with_admin_markup,$cc_id);
...
$with_admin_markup = (($markup_org /100) * $with_admin)+$with_admin;
	...
	$this->voucher_email($val_last);	
	redirect('cc_hotel/voucher/'.$val_last, 'refresh');	//redirect('cc_hotel/voucher/'.$val_last, 'refresh');		
...
';
	...
	//print_r($data);exit;
		 ...
		 $this->load->view('cc/hotel/voucher',$data);
	  }
	  else
	  {
		redirect('cc_hotel/index','refresh');
