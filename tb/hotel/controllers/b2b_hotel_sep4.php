...
public function backtosearch()
{
	...
	public function search()
	{
		...
		$data['selected_api'] = $this->B2b_Hotel_Model->getAPIs();
		$this->load->view('b2b/hotel/search_result',$data);
		}
		else
		{
			//$this->load->view('b2b/hotel/agent_page');
			redirect('b2b/agent_page', 'refresh');
		}
		}
	}
...
$this->session->set_userdata($newdata);
	...
	public function fetch_search_result() 
	{
		...
		$max_val = ceil($tmp_data['maxVal']*$this->session->userdata('currency_value'));
		$tot_rec = $tmp_data['totRow'];
  ...
  if(isset($parser->document->responsedetails[0]->searchhotelpriceresponse[0]->hoteldetails[0]->hotel))
  {
			foreach($parser->document->responsedetails[0]->searchhotelpriceresponse[0]->hoteldetails[0]->hotel as $hotels)
			{
				...
				foreach($hotels->roomcategories[0]->tagChildren as $roomcategory)
				{
					...
					$this->B2b_Hotel_Model->insert_gta_temp_result($sec_res,'gta',$code,$roomcategory->tagAttrs['id'],$roomcategory->description[0]->tagData,$roomcategory->itemprice[0]->tagData,$roomcategory->confirmation[0]->tagData,$roomcategory->meals[0]->basis[0]->tagData,$adult,$child,$city_id);
				}
				$rows++;
	...
	function my_gta_hotel_availabilty()
	{
		echo "aaa"; 
		$room_used_type=$_SESSION['room_used_type'];
		$city=$_SESSION['city'];
...
$api='gta';
				...
				foreach($RoomFacilities as $RoomFacilitiessal)
				{
					...
					foreach($Facility as $Facilityssss)
					{
						...
						$this->B2b_Hotel_Model->insert_room_fac_hotel($itemCode,$Facility_val);
						$j++;
				...
				foreach($Facilities as $Facilitiesaaa)
				{
					...
					foreach($Facility as $Facilityaaaa)
					{
						...
						$this->B2b_Hotel_Model->insert_room_fac_hotel1($itemCode,$Facility_val);
						$h++;
			...
			if($Image_val !='')
			{
				...
				$this->B2b_Hotel_Model->insert_permenant_result_pro($itemCode,$itemVal,$City_val,$star,$report_value,$location_value,$lati,$longi,$api,$address,$tel_val,$fax_val);
...
$api='gta';
	...
	//	echo $hotelcode;exit;
		 ...
		 for($l=0;$l< $hotel_count;$l++)
		 {
			...
			$this->B2b_Hotel_Model->insert_permenant_result_pro($hotelcode,$name,$city,$star,$descval,$location_value,$lati,$longi,$api,$address,$phoneval,$faxval,$Chainval,$imageval);
			  }
		 }
...
//echo $sb_room_cnt.'--'.$db_room_cnt;exit;
  ...
  foreach ((array)$hotelResponse as $hnum => $hotel) 
  {
	...
	$api="hotelspro";
      ...
      $sec_res=$_SESSION['ses_id'];
				$this->B2b_Hotel_Model->insert_gta_temp_result($sec_res,'hotelspro',$hotelCode,$processId,$roomtype,$totalRoomRate,$availabilityStatus,$boardType);   
					...
					//print_r($xmls);exit;
					 ...
					 $dom->loadXML($xmls);   
					  ...
					  foreach($sh as $val)
					  {
  				 	...
  				 	$sec_res=$_SESSION['ses_id'];
					$this->B2b_Hotel_Model->insert_hotelsbed_temp_result($sec_res,'hotelsbed',$codev1,$roomTypeVal,$roomv1,$amountv1,'Available',$boardv1,$shruiVal,$charVal,$adult,$child,$boardTypeVal,$token,$incode,$inoffcode,$contractnameVal,$destCodeVal,$shortname,$city_id);  						   
...
$URL = "http://xmlv6.travco.co.uk/trlink/link1/trlink";//live
		...
		curl_close($ch);
			...
			if($CURRENCY_CODE!='USD')
			{
				...
				$cur_val1=$this->B2b_Hotel_Model->get_currecy_detail($CURRENCY_CODE);
		    ...
		    $sec_res=$_SESSION['ses_id'];
			  ...
			  $adult = $room_pax;
			  $child = 0;
			$this->B2b_Hotel_Model->insert_gta_temp_result_travco($sec_res,'travco',$hotelCode,$roomcode,$roomTypeVal,$totalprice,$STATUS,$inclustion,$adult,$child,$cancelCode);
	...
	function hotel_detail($id)
	{
		...
		else
		{
         ...
         //	echo "<pre/>";
		//print_r($_SESSION['fav_hotel_detail']);exit;
...
$agent_id = $this->session->userdata('agent_id');
  ...
  $api_id = $this->B2b_Hotel_Model->getAPI_ID($value['api']);
  $data5 = array(
   'product_id' => 2,
   'user_id' => $agent_id,
   'amount' => $value['amount'],
   'net_amount' => $value['net_price'],
  ...
  // 'status' => 'confirmed',
	...
	//$this->booking_final_pay($id,$parent_transaction_id);
	 if ($this->B2b_Hotel_Model->debitBookingAmount($agent_id,$branch_id,$value['net_price'])) {
...
//print_r($_SESSION);exit;
	...
	//echo $data;
		...
		//echo $xmls;exit;
			...
			$IncomingOfficecode='';
			foreach($IncomingOffice as $aaaaaaaa)
			{
...
$dateFromValc='';
 ...
 $this->B2b_Hotel_Model->inser_customer_book_hotelpro_trans_hotel($con_id_org,$bookingItemCodeval,$user_id,$val_last,$IncomingOfficecode);
...
$hotelcodeval = $hotel_id;
 ...
 $this->B2b_Hotel_Model->inser_customer_book_hotelpro_trans_hotel($trans_id,$bookingItemCodeval,$user_id,$val_last,$book_noval);
		$this->voucher_email($val_last);	
...
';
  ...
  /*$agent_acc_info = $this->B2b_Model->getAgentAccInfo($agent_id);
   ...
   if ($agent_type == 2) { 
		...
		$agent_acc_info = $this->B2b_Model->getBranchAccInfo($agent_id, $branch_id);
		if (!empty($agent_acc_info)) {
		$data['balance_credit'] = $agent_acc_info[0]->balance_credit;
		$data['last_credit'] = $agent_acc_info[0]->last_credit;
		}
	}
	...
	function fetch_other_price()
	{
		$room_count = $_SESSION['room_count'];
		//echo "rr:".$room_count;
		$hotel_code = $_REQUEST['hotel_code'];
		$api_temp_hotel_id = $_REQUEST['api_temp_hotel_id'];
		$service=$this->B2b_Hotel_Model->get_searchresult($api_temp_hotel_id);
		
		$data['result'] = array();
		if ($room_count == 1 || $service->api == 'gta') {
			$tmp_data = $this->B2b_Hotel_Model->fetch_gta_temp_result_room($_SESSION['ses_id'],$hotel_code);
			$data['result'] = $tmp_data;
		} else {
			//$service=$this->B2b_Hotel_Model->get_searchresult($api_temp_hotel_id);
			
			$merge_inclsuion = $this->B2b_Hotel_Model->get_merge_inclsuion_hotelsbed($service->hotel_code,$service->api,$service->session_id);
			...
			for($tt=0;$tt< count($merge_inclsuion);$tt++)
			{
						...
						$strings_inc = $merge_inclsuion[$tt]->inclusion;  
						 $merge_inclsuion_inc = $this->B2b_Hotel_Model->get_merge_inclsuion_hotelsbed_inc($service->hotel_code,$service->api,$service->session_id,$strings_inc);
			...
			$a = $this->B2b_Hotel_Model->concatss($strings, "");
			...
			for($ss=0;$ss<count($b);$ss++)
			{
					...
					if($c_c !='')
					{
						...
						$merge_result_id='';
							...
							$merge_inclsuionroom = $this->B2b_Hotel_Model->get_merge_inclsuion_hotelsbed_mapping($service->hotel_code,$service->api,$service->session_id,$cut_expl[$ii]);
			...
			$this->B2b_Hotel_Model->insert_hotel_detail_mapping($service->session_id,$merge_room_type,$merge_inclusion,$_SESSION['days'],$merger_total_cost.' USD',$merge_status,$merge_result_id1);
				
				}
			} // for($i=0;$i<count($a);$i++)
		
			$row = $this->B2b_Hotel_Model->get_hotel_detail_mapping($service->session_id);
			//print_r($row);
			$other_price = array();
			for($id=0;$id<count($row);$id++)
			{
					$other_price[$id]['room_type'] = $row[$id]->room;
					$other_price[$id]['inclusion'] = $row[$id]->inc;
					$other_price[$id]['total_cost'] = $row[$id]->cost;
					$other_price[$id]['days'] = $row[$id]->days;
					$other_price[$id]['status'] = $row[$id]->status;
					$other_price[$id]['api_temp_hotel_id'] = $row[$id]->id;
			}
			$data['result'] = $other_price;
		}
