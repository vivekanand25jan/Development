	...
	public function backtosearch()
{
	...
	{
		if (!$this->session->userdata('cc_logged_in')) {
			redirect('cc/user_login', 'refresh'); 
		}
		$data = array(
			'status' => 'Closed',
			'process' => 'Call Center',
		...
		redirect('cc/support_ticket','refresh');
	}
	...
	$data['agent_list'] = $this->B2b_Model->getsupportinfo_p($cc_id);
//	$data['agent_list_h'] = $this->B2b_Model->getsupportinfo($cc_id,'On Hold');
	$data['agent_list_c'] = $this->B2b_Model->getsupportinfo_c($cc_id);
	$this->load->view('cc/support_ticket',$data);
	...
	function view_ticket($id)
	{
		...
		$this->load->view('cc/view_ticket',$data);
	...
	function update_ticket()
	{
		...
		$this->B2b_Model->update_ticket($agent_id,$ticket_id,$message);
				redirect('cc/view_ticket/'.$ticket_id ,'refresh');
	}
...
$this->email->send();
	...
	function login()
	{
		...
		if ($this->session->userdata('cc_logged_in')) {
			redirect('cc/cc_page', 'refresh'); 
		} else {
			$this->load->view('cc/login_page');
		}
	...
	function user_login()
	{
		...
		if($this->form_validation->run()==FALSE)
		{
			$data['status'] = "";
			 ...
			 $this->load->view('cc/login_page',$data);
		}
		else
		{
			 ...
			 $userValied = $this->B2b_Model->check_agent_login_cc($email, $pass);
...
echo '</table>';
	...
	//echo '<pre/>';
	//print_r($this->session->userdata);exit;
			$data['cc_acc_info'] = $this->B2b_Model->getAgentAccInfo_cc($cc_id);
			
		
		$data['agent_list'] = $this->B2b_Model->getAgentlist_cc($cc_id);
		
			$data['cc_history'] = $this->B2b_Model->getAgentAccInfo_cc_balance($cc_id);
		$parent_agent=0;
	
		//$data['agentTopCities'] = $this->B2b_Model->getAgentTopCities($parent_agent);
	//	echo '<pre/>';
	//	print_r($data);exit;
		$this->load->view('cc/cc_index', $data);
	}
...
<ExtraBeds>
  ...
  if(isset($parser->document->responsedetails[0]->searchhotelpriceresponse[0]->hoteldetails[0]->hotel))
  {
		 ...
		 //  $currency = $this->input->post('currency');
		   $email_id = $this->input->post('email_id');
			...
			if ($this->B2b_Model->edit_agent($agent_id, $name, $company_name, $address, $country, $city, $postal_code, $office_phone, $mobile, $email_id)) {
					...
					//$data['emailId'] = $email;
					redirect('/cc/agent_profile', 'refresh');
	...
	function __password_change()
	{
		...
		$this->load->view('cc/password_change',$data);
	}
	...
	function edit_branch($branch_id)
	{
		...
		if (empty($data['result']->branch_id)) {
			redirect('cc/branch_manage', 'refresh');
		} else {
	...
	function staff_manage()
	{
		...
		$data['txt_search'] = $txt_search;
		$this->load->view('cc/staff_manage',$data);
	}
	...
	function __add_staff()
	{
		...
		$this->load->view('cc/add_staff', $data);
	}
		...
		if($this->form_validation->run()==FALSE)
		{
		   ...
		   $data['status'] = $this->input->post('status');
			...
			if (empty($data['result']->staff_id)) {
				$data['branchs'] = $this->B2b_Model->getBranchs($agent_id);
				$this->load->view('cc/edit_staff',$data);
	...
	{	
		...
		$data['result'] = $this->B2b_Model->getBranchDepositInfo($deposit_id, $agent_id);
			...
			if (empty($data['result']->deposit_id)) {
				redirect('cc/branch_deposit_manage', 'refresh');
			} else {
				$data['branchs'] = $this->B2b_Model->getBranchs($agent_id);
	...
	function add_branch_markup_process()
	{
		...
		else
		{
		   ...
		   if ($this->B2b_Model->add_branch_markup($branch, $markup)) {
					...
					redirect('cc/branch_markup_manage', 'refresh');
...
echo '</table>';
	...
	function my_booking()
	{
		...
		$branch_id = $this->session->userdata('branch_id');
			...
			//echo '<pre/>';
			//print_r($data);exit;
				 $this->load->view('cc/my_booking',$data);
	}
	function my_booking_search_news()
	{
...
<ExtraBeds>
	...
	//$this->booking_final_pay($id,$parent_transaction_id);
		redirect('cc/booking_final_pay/'.$id.'/'.$parent_transaction_id, 'refresh');
...
$dateFromValc='';
 ...
 $this->B2b_Hotel_Model->inser_customer_book_hotelpro_trans_hotel($trans_id,$bookingItemCodeval,$user_id,$val_last);
		$this->voucher_email($val_last);	
			...
			redirect('cc/voucher/'.$val_last, 'refresh');		
	...
	function send_voucher_email($id,$msg='')
{
	...
	//$data =$_SESSION['voucher_data'];
		...
		redirect('cc/send_voucher_email/'.$id.$msg, 'refresh');
	}
