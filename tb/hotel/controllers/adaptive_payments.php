...
class Adaptive_payments extends CI_Controller 
{
	...
	function Pay_chained_demo()
	{
		...
		$pay_res_id			= $_POST['pay_res_id'];
		$parent_transaction_id	= $_POST['parent_transaction_id'];
		$paid_id = '1';
		/*$car_api_id			= $_POST['car_api_id'];
		$email				= $_POST['email'];
		$city				= $_POST['city'];
		...
		$usdAmt				= round($this->ConverCurrency($grand_total,$currency,'USD'),2);
		$booked_date 		= date("Y-m-d");
		$username 			= $_POST['username'];
		$car_payAmt 		= $usdAmt;
		$car_api_id 		= $car_api_id;
		// echo $;exit;
		
		$check_data = $this->Car_Model->get_car_pay_details_by_para($car_api_id, $username);

		if(empty($check_data)) {
			// echo 'sdsds';
			$insId	= $this->Car_Model->insert_into_car_pay_details($user_id,$car_api_id,$username,$car_payAmt, $email, $city, $zip_code, $state, $mobile_no, $office_phone, $booked_date);
			$car_paid_id = $insId;
		} else {
			$car_paid_id	= $check_data[0]->id;
			if($check_data[0]->amount != $usdAmt) {
				$update_data = $this->Car_Model->update_car_pay_details($car_paid_id,$usdAmt);
			}
		...
		$PayRequestFields = array(
								...
								'CancelURL' => site_url('b2b_hotel/pay_response_fail'), 									// Required.  The URL to which the sender's browser is redirected if the sender cancels the approval for the payment after logging in to paypal.com.  1024 char max.
								...
								'Pin' => '', 										// The sener's personal id number, which was specified when the sender signed up for the preapproval
								'PreapprovalKey' => '', 							// The key associated with a preapproval for this payment.  The preapproval is required if this is a preapproved payment.  
								'ReturnURL' => site_url('b2b_hotel/success_pay/'.$pay_res_id.'/'.$parent_transaction_id), 	// Required.  The URL to which the sener's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
								//'ReturnURL' => site_url('adaptive_payments/success_pay/'.$car_paid_id), 	// Required.  The URL to which the sener's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
								'ReverseAllParallelPaymentsOnError' => '', 			// Whether to reverse paralel payments if an error occurs with a payment.  Values are:  TRUE, FALSE
								'SenderEmail' => '', 								// Sender's email address.  127 char max.
								'TrackingID' => ''									// Unique ID that you specify to track the payment.  127 char max.
								);
		...
		$Receivers = array();
		$Receiver = array(
						'Amount' => $total_amount, 											// Required.  Amount to be paid to the receiver.
						'Email' => 'elizabeth@travel-bay.com', 												// Receiver's email address. 127 char max.
						'InvoiceID' => $paid_id, 											// The invoice number for the payment.  127 char max.
						'newfield' => 'NEW', 											// The invoice number for the payment.  127 char max.
						'PaymentType' => 'SERVICE', 										// Transaction type.  Values are:  GOODS, SERVICE, PERSONAL, CASHADVANCE, DIGITALGOODS
						'Quantity' => '34', 										// Transaction type.  Values are:  GOODS, SERVICE, PERSONAL, CASHADVANCE, DIGITALGOODS
						'PaymentSubType' => '', 									// The transaction subtype for the payment.
						'Phone' => array('CountryCode' => '', 'PhoneNumber' => '', 'Extension' => ''), // Receiver's phone number.   Numbers only.
		...
		$PayPalRequestData = array(
							...
							'SenderIdentifierFields' => $SenderIdentifierFields, 
							'AccountIdentifierFields' => $AccountIdentifierFields
							);	//$FundingTypes 							'FundingTypes' =>$FundingTypes, 
		$PayPalResult = $this->paypal_adaptive->Pay($PayPalRequestData);
//print_r($this->paypal_adaptive->APICallSuccessful($PayPalResult['Ack'])); die;
		...
		else
		{
			// Successful call.  Load view or whatever you need to do here.
				// 
				// echo '<pre>';print_r($PayPalResult);echo '</pre>';
		// exit;	
	...
	function Pay_chained_hotel()
	{
		...
		$api_temp_hotel_id			= $_POST['api_temp_hotel_id'];
		$usdAmt						= round($this->ConverCurrency($grand_total,'INR','USD'),2);
		$booked_date 				= date("Y-m-d");
		
		$check_data = $this->Hotel_Model->get_hotel_pay_details_by_para($api_temp_hotel_id,$fname,$email);
		//echo '<pre>';print_r($check_data);echo '</pre>';exit;
		
		if(empty($check_data)) {
		
		$insId	= $this->Hotel_Model->insert_into_hotel_pay_details($user_id,$api_temp_hotel_id,$fname,$usdAmt,$email,$lname,$city,$zip_code,$state,$mobile_no,$office_phone,$fax,$booked_date);
		
			$h_cus_contact_details_id	= $insId;
		
		} else {
			
			$h_cus_contact_details_id	= $check_data[0]->h_cus_contact_details_id;
			
				$update_data = $this->Hotel_Model->update_hotel_pay_details($h_cus_contact_details_id,$user_id,$fname,$usdAmt,$email,$lname,$city,$zip_code,$state,$mobile_no,$office_phone,$fax);
		...
		$PayRequestFields = array(
								'ActionType' => 'PAY', 								// Required.  Whether the request pays the receiver or whether the request is set up to create a payment request, but not fulfill the payment until the ExecutePayment is called.  Values are:  PAY, CREATE, PAY_PRIMARY
								'CancelURL' => site_url('hotel/checkout_page_detail/'.$api_temp_hotel_id), 									// Required.  The URL to which the sender's browser is redirected if the sender cancels the approval for the payment after logging in to paypal.com.  1024 char max.
								'CurrencyCode' => 'USD', 							// Required.  3 character currency code.
								'FeesPayer' => 'EACHRECEIVER', 						// The payer of the fees.  Values are:  SENDER, PRIMARYRECEIVER, EACHRECEIVER, SECONDARYONLY
								'IPNNotificationURL' => '', 						// The URL to which you want all IPN messages for this payment to be sent.  1024 char max.
								'Memo' => '', 										// A note associated with the payment (text, not HTML).  1000 char max
								'Pin' => '', 										// The sener's personal id number, which was specified when the sender signed up for the preapproval
								'PreapprovalKey' => '', 							// The key associated with a preapproval for this payment.  The preapproval is required if this is a preapproved payment.  
								'ReturnURL' => site_url('hotel/success_pay/'.$h_cus_contact_details_id.'/'.$api_temp_hotel_id), 	// Required.  The URL to which the sener's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
								'ReverseAllParallelPaymentsOnError' => '', 			// Whether to reverse paralel payments if an error occurs with a payment.  Values are:  TRUE, FALSE
								'SenderEmail' => '', 								// Sender's email address.  127 char max.
								'TrackingID' => ''									// Unique ID that you specify to track the payment.  127 char max.
								);
		...
		$Receivers = array();
		$Receiver = array(
						'Amount' => $usdAmt, 											// Required.  Amount to be paid to the receiver.
						'Email' => 'momahim@gmail.com', 												// Receiver's email address. 127 char max.
						'InvoiceID' => '845625565', 											// The invoice number for the payment.  127 char max.
						'newfield' => 'NEW', 											// The invoice number for the payment.  127 char max.
						'PaymentType' => 'SERVICE', 										// Transaction type.  Values are:  GOODS, SERVICE, PERSONAL, CASHADVANCE, DIGITALGOODS
						'Quantity' => '34', 										// Transaction type.  Values are:  GOODS, SERVICE, PERSONAL, CASHADVANCE, DIGITALGOODS
						'PaymentSubType' => '', 									// The transaction subtype for the payment.
	...
	function Pay_chained_villa()
	{
		...
		$fax 						= $_POST['fax'];
		$grand_total				= $_POST['grand_total'];
		$api_temp_villa_id			= $_POST['api_temp_villa_id'];
		$booked_date 				= date("Y-m-d");
		$usdAmt						= round($this->ConverCurrency($grand_total,'INR','USD'),2);
		
		$check_data = $this->Villa_Model->get_villa_pay_details_by_para($api_temp_villa_id,$fname,$email);
		//echo '<pre>';print_r($check_data);echo '</pre>';exit;
		
		if(empty($check_data)) {
		
		$insId	= $this->Villa_Model->insert_into_villa_pay_details($user_id,$api_temp_villa_id,$fname,$usdAmt,$email,$lname,$city,$zip_code,$state,$mobile_no,$office_phone,$fax,$booked_date);
		
			$v_cus_pay_details_id	= $insId;
		
		} else {
			
			$v_cus_pay_details_id	= $check_data[0]->v_cus_pay_details_id;
			
		//echo $v_cus_pay_details_id;exit;
				$update_data = $this->Villa_Model->update_villa_pay_details($v_cus_pay_details_id,$user_id,$fname,$usdAmt,$email,$lname,$city,$zip_code,$state,$mobile_no,$office_phone,$fax);
		...
		$PayRequestFields = array(
								'ActionType' => 'PAY', 								// Required.  Whether the request pays the receiver or whether the request is set up to create a payment request, but not fulfill the payment until the ExecutePayment is called.  Values are:  PAY, CREATE, PAY_PRIMARY
								'CancelURL' => site_url('villa/checkout_page_detail/'.$api_temp_villa_id), 									// Required.  The URL to which the sender's browser is redirected if the sender cancels the approval for the payment after logging in to paypal.com.  1024 char max.
								'CurrencyCode' => 'USD', 							// Required.  3 character currency code.
								'FeesPayer' => 'EACHRECEIVER', 						// The payer of the fees.  Values are:  SENDER, PRIMARYRECEIVER, EACHRECEIVER, SECONDARYONLY
								'IPNNotificationURL' => '', 						// The URL to which you want all IPN messages for this payment to be sent.  1024 char max.
								'Memo' => '', 										// A note associated with the payment (text, not HTML).  1000 char max
								'Pin' => '', 										// The sener's personal id number, which was specified when the sender signed up for the preapproval
								'PreapprovalKey' => '', 							// The key associated with a preapproval for this payment.  The preapproval is required if this is a preapproved payment.  
								'ReturnURL' => site_url('villa/success_pay/'.$v_cus_pay_details_id.'/'.$api_temp_villa_id), 	// Required.  The URL to which the sener's browser is redirected after approvaing a payment on paypal.com.  1024 char max.
								'ReverseAllParallelPaymentsOnError' => '', 			// Whether to reverse paralel payments if an error occurs with a payment.  Values are:  TRUE, FALSE
								'SenderEmail' => '', 								// Sender's email address.  127 char max.
								'TrackingID' => ''									// Unique ID that you specify to track the payment.  127 char max.
								);
