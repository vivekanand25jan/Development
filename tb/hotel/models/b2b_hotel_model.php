...
function get_markup_detail($contry,$sup_hotel_id,$b2b_user_id)
{
		 ...
		 function xml2array($xmlStr, $get_attributes = 1, $priority = 'tag')
	{
		...
		*/
		...
		function inser_customer_book_hotelpro_trans_hotel($trans_id,$ConfirmationNumbervalue,$userid,$val_last,$status='',$amt='')
		{
			$ss=$_SESSION['ses_id'];
			...
			{
				$this->db->query("UPDATE transaction_details SET prn_no='$m', 	booking_number='$ConfirmationNumbervalue',  user_id='$userid' , hotel_booking_id='$val_last',  user_id='$userid',  status='$status' , session='$ss',acc_balance=$amt  WHERE customer_contact_details_id ='$trans_id'");
			}
...
WHERE t.hotel_code = p.hotel_code  AND p.api = 'crs' AND t.api_temp_hotel_id='".$id."'");
...
//		echo $this->db->last_query();exit;
	 ...
	 function Update_room_type_in_search_result($result_id,$hotel_proid,$amountv4,$room,$amountv3 ,$boardType,$org_amt,$currencyv1,$c_val,$amountv3pay)
 {
	...
	function pass_info_detail1($tran_id)
	{
		$result = "";
		$query1 ="";
		$que = "";
	...
	{
		//echo "hello";
		$select = "SELECT *,A.status as statuss FROM sup_apart_maintain_month A 
	...
	function get_hotel_daily_allotment($hotel_id, $priceDate,$priceDate1, $room_type, $season_id)
	{
		...
		$room_typee = explode('-',$room_type);
		$room_typee[0]; 
		//print_r($season_id);
		$season_id1 = explode('-',$season_id);
		for($kk=0;$kk<count($season_id1);$kk++)
		{
			$ss = ($season_id1[$kk]!='')?$season_id1[$kk]:$season_id;
		 $sql="SELECT * FROM sup_inv_cate_allotment 
		WHERE hotel_id='".$hotel_id."' AND season_id='".$ss."'
		 AND cat_type ='".$room_typee[0]."'
	...
	function get_promotion_dates($sup_rate_tactics_id,$hotel_id)
	{
		$select = "SELECT promotion_avail_date_from,promotion_avail_date_to FROM promotions 
					WHERE hotel_id = '".$hotel_id."' 
	...
	function get_hotel_promotion_stay_pay($start,$days,$s_date, $e_date, $hotel_id, $sup_rate_tactics_id)
	{
		$availdates=$this->get_promotion_dates($sup_rate_tactics_id,$hotel_id);
		
		$_SESSION['counterdays']='';
		$availdates=$this->get_promotion_dates($sup_rate_tactics_id,$hotel_id);
	 $promo_from_date=$availdates[0]->promotion_avail_date_from;
	 $promo_to_date=$availdates[0]->promotion_avail_date_to;
		$counterdays=0;
		for($i=0;$i<=$days;$i++)
		{
			//echo $start;echo "  ";
		...
		$select = "SELECT * FROM promotions 
					WHERE hotel_id = '".$hotel_id."' 
						AND room_tactics_id = '".$sup_rate_tactics_id."'
						AND '".$date."' BETWEEN promotion_avail_date_from AND promotion_avail_date_to
						AND status = '1' ";  
		//echo $select;				
		$query=$this->db->query($select);
	...
	$_SESSION['counterdays']=$counterdays;
	...
	function reduce_allotment($h_hotel_id, $h_room_code, $h_season, $cin, $cout)
	{
        ...
        public  function getExtra($hotel_id){
            
            $query = $this->db->get_where('hotel_booking_info',array('hotel_booking_info_id'=>$hotel_id));
            //echo $this->db->last_query(); die;
            if($query->num_rows()>0){
                $result = $query->result_array();
                return $result[0]['extra_services'];
            }else{
                return '';
            }
        }
