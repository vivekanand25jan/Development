   ...
   public function add_promotion($supplier_id, $hot_id, $room_tac_id, $room_avail_date_from, $room_avail_date_to, $staynights, $paynights, $bbdate, $hhdate, $promotion_avail_from, $promotion_avail_to, $dates, $weekday, $price, $extra_bed_price, $avail, $aadult, $achild, $child_price, $infant, $sup_charge,$booking_code,$sup_room_avail_date_from,$sup_room_avail_date_to, $season_id,$category_type)
	{
		...
		$sql="INSERT INTO promotions(promo_id,room_tactics_id,hotel_id,from_date,to_date,stay_nights,pay_nights,bb_date,hh_date,promotion_avail_date_from,promotion_avail_date_to,booking_code,status) VALUES('', '$room_tactics_id' ,'$hot_id','$room_avail_date_from','$room_avail_date_to','$staynights','$paynights','$bbdate', '$hhdate','$promotion_avail_from','$promotion_avail_to','$booking_code','1')";
		//echo $sql;die;
		$rs=$this->db->query($sql);
                $id = $this->db->insert_id();
	...
	function update_promotion($id,$status)
	{
		...
		echo	$select = "SELECT * FROM sup_inv_cate_allotment WHERE sup_id = '$supplier_id' AND hotel_id = '$id' GROUP BY sup_id, hotel_id, cat_type, season_id, allocation_validity_from, allocation_validity_to";
			$query=$this->db->query($select);
			if ($query->num_rows() > 0)
			{
				return $query->result();
			...
			$allocation_release_period, $allocation_validity_from, $allocation_validity_to)
		{
			...
			$update_query="UPDATE sup_inv_cate_allotment SET cat_type='$cate_type', season_id='$season_id', allocation_rooms='$allocation_rooms', allocation_release_days='$allocation_release_days', allocation_release_period='$allocation_release_period', allocation_validity_from='$allocation_validity_from', allocation_validity_to='$allocation_validity_to', status='1' WHERE sup_inv_cat_allotment_id='$id'";
			
			//print '<pre>';print_r($update_query);exit;
		...
		function update_allotment_list($id,$status)
		{
			if($status == 2)
			{
				$where2 = "sup_inv_cat_allotment_id = $id";
			...
			{
				$where = "sup_inv_cat_allotment_id = $id";
				$data = array(
					'status' => $status
				);
	...
	function update_promotion_rates($supplier_id, $hot_id, $room_tac_id,$promo_id, $fromdate, $todate, $staynights, $paynights, $bbdate, $hhdate, $promotion_avail_from, $promotion_avail_to, $dates,$datess, $weekday, $price, $extra_bed_price, $avail, $aadult, $achild, $child_price, $infant, $sup_charge,$booking_code)
	{
		...
		//echo $transferto;exit;
		$sql="INSERT INTO promotions(promo_id,room_tactics_id,hotel_id,from_date,to_date,stay_nights,pay_nights,bb_date,hh_date,promotion_avail_date_from,promotion_avail_date_to,booking_code,status) VALUES('', '$room_tac_id' ,'$hot_id','$fromdate','$todate','$staynights','$paynights','$bbdate', '$hhdate','$promotion_avail_from','$promotion_avail_to','$booking_code','1')";
		$rs=$this->db->query($sql);
                $id = $this->db->insert_id();
